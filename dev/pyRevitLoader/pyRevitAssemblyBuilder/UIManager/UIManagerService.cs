...
namespace pyRevitAssemblyBuilder.UIManager
{
    ...
    public class UIManagerService : IUIManagerService
    {
        ...
        /// <summary>
        /// Emits the per-extension ribbon counter lines (post-processing, icons, AddItem)
        /// collected during the most recent <see cref="BuildUI"/> call. Called by the session
        /// manager immediately after the wrapping <c>[PERF] {ext.Name} - BuildUI: Xms</c> line
        /// so the detail sits underneath it in the log.
        /// </summary>
