...
sys.path.append(
    op.join(
        userdesktop, r"gits\pyRevitLabs\pyRevitLabs\pyRevitLabs.DeffrelDB\bin\Debug"
    )
)
...
# play with records ===========================================================
STRINGS = ["Example Keynote Text", 'Example "Keynote" Text']
...
for record_log in ADDED_RECORDS:
    ...
    assert rfields["record_text"] == record_log[3], (
        'Failed record text matching: {} "{}" != "{}"'.format(
            record_log[2], rfields["record_text"], record_log[3]
        )
    )
print(
    "[  OK  ] %s passed all read tests for %s records" % (requester, len(ADDED_RECORDS))
)
