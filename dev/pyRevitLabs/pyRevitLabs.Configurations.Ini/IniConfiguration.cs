...
public sealed class IniConfiguration : ConfigurationBase {
    ...
    private static readonly Regex LegacyPathSeparatorRegex = new(@"[A-Za-z]:\\(?!\\)", RegexOptions.Compiled);

    /// <summary>
    /// Converts legacy Windows path separators into JSON escapes.
    /// </summary>
    ...
    private static bool TryJsonStringList(string raw, out List<string>? list) {
        ...
        string value = LegacyPathSeparatorRegex.IsMatch(raw) ? EscapeBackslashes(raw) : raw;
