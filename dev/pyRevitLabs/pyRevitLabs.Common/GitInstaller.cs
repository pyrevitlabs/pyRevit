...
namespace pyRevitLabs.Common
{
    ...
    public static class GitInstaller
    {
        ...
        // @handled @logs
        public static Repository Clone(string repoPath, string branchName, string destPath, string username, string password,
            bool checkout = true)
        {
            // build options and clone
            var cloneOps = new CloneOptions() { Checkout = checkout, BranchName = branchName };

            // Add username and password if user provided them
            if (username != null && password != null)
            {
                cloneOps.CredentialsProvider = (_url, _user, _cred) =>
