...
// to help the CLR find the appropriate pyRevitLabs.PythonNet assembly.

// If defined, the "pythonRuntimeVersionString" variable must be set to
// pyRevitLabs.PythonNet's current version.
#define USE_PYTHON_RUNTIME_VERSION

// If defined, the "PythonRuntimePublicKeyTokenData" data array must be
// set to pyRevitLabs.PythonNet's public key token. (sn -T Python.Runtin.dll)
...
#endif
    {
        DebugPrint("Attempting to load 'pyRevitLabs.PythonNet' using standard binding rules.");
        ...
        // Attempt to find and load pyRevitLabs.PythonNet using standard assembly binding rules.
        ...
        var pythonRuntimeName = new AssemblyName("pyRevitLabs.PythonNet")
        {
        ...
        try
        {
            ...
            DebugPrint("Success loading 'pyRevitLabs.PythonNet' using standard binding rules.");
        }
        catch (IOException)
        {
            DebugPrint("'pyRevitLabs.PythonNet' not found using standard binding rules.");
            try
            {
                // If the above fails for any reason, we fallback to attempting to load "pyRevitLabs.PythonNet.dll"
                ...
                string pythonRuntimeDllPath = Path.Combine(assemblyDirectory, "pyRevitLabs.PythonNet.dll");
                DebugPrint($"Attempting to load pyRevitLabs.PythonNet from: '{pythonRuntimeDllPath}'.");
                pythonRuntime = Assembly.LoadFrom(pythonRuntimeDllPath);
                DebugPrint($"Success loading 'pyRevitLabs.PythonNet' from: '{pythonRuntimeDllPath}'.");
            }
            catch (InvalidOperationException)
            {
                DebugPrint("Could not load 'pyRevitLabs.PythonNet'.");
#if PYTHON3
                return IntPtr.Zero;
#elif PYTHON2
                return;
        ...
        // Once here, we've successfully loaded SOME version of pyRevitLabs.PythonNet
        // So now we get the PythonEngine and execute the InitExt method on it.
        Type pythonEngineType = pythonRuntime.GetType("pyRevitLabs.PythonNet.PythonEngine");
