...
def scope_as_string(wbook, scope):
    if 0 <= scope < wbook.nsheets:
        return "sheet #%d (%r)" % (scope, REPR(wbook.sheet_names()[scope]))
    if scope == -1:
        return "Global"
    if scope == -2:
        return "Macro/VBA"
...
def do_scope_query(wbook, scope_strg, show_contents=0, f=sys.stdout):
    try:
        qscope = int(scope_strg)
    except ValueError:
        if scope_strg == "*":
            qscope = None  # means all scopes
        else:
            # so assume it's a sheet name ...
            qscope = wbook.sheet_names().index(scope_strg)
            print("%r => %d" % (scope_strg, qscope), file=f)
    for nobj in wbook.name_obj_list:
        if qscope is None or nobj.scope == qscope:
            show_name_object(wbook, nobj, show_contents, f)


def show_name_details(wbook, name, show_contents=0, f=sys.stdout):
    """
    ...
    nobj_list = wbook.name_map.get(name_lcase)
    if not nobj_list:
...
def show_name_details_in_scope(
        wbook, name, scope_strg, show_contents=0, f=sys.stdout,
):
    try:
        scope = int(scope_strg)
    except ValueError:
        # so assume it's a sheet name ...
        scope = wbook.sheet_names().index(scope_strg)
        print("%r => %d" % (scope_strg, scope), file=f)
    name_lcase = name.lower()  # Excel names are case-insensitive.
    while 1:
        nobj = wbook.name_and_scope_map.get((name_lcase, scope))
        if nobj:
            break
...
def show_name_object(wbook, nobj, show_contents=0, f=sys.stdout):
    print("\nName: %s, scope: %s (%s)"
          % (REPR(nobj.name), REPR(nobj.scope), scope_as_string(wbook, nobj.scope)), file=f)
    res = nobj.result
    ...
    if kind >= 0:
        ...
        for i in range(len(value)):
            ref3d = value[i]
            print("Range %d: %s ==> %s" % (
                i, REPR(ref3d.coords), REPR(xlrd.rangename3drel(wbook, ref3d))), file=f)
    elif kind == xlrd.oREF:
        # A list of Ref3D objects
        for i in range(len(value)):
            ref3d = value[i]
            print("Range %d: %s ==> %s" % (
                i, REPR(ref3d.coords), REPR(xlrd.rangename3d(wbook, ref3d))), file=f)
            if not show_contents:
                continue
            datemode = wbook.datemode
            for shx in range(ref3d.shtxlo, ref3d.shtxhi):
                sh = wbook.sheet_by_index(shx)
...
"""
    ...
    for fname in glob.glob(arg_pattern):
        ...
        if arg_name == "*":
            ...
            # Using book.name_and_scope_map to find which is any instances
