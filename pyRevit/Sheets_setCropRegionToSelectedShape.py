...
def sortcurvescontiguous(origcurves):
	...
	n = len(curves)
	if DEBUG: print('NUMBER OF CURVES: {0}'.format(n))
	...
	for i in range(0,n):
		...
		for j in range(i+1,n):
			...
			if DEBUG: print('END2START: {0}'.format(_sixteenth > p.DistanceTo(endPoint)))
			if _sixteenth > p.DistanceTo(endPoint):
				if i + 1 != j:
					tmp = curves[i + 1]
					curves[i + 1] = curves[j]
					curves[j] = tmp
					if DEBUG: print('SWAPPED.')
				if DEBUG: print('SWAP UNECESSARY.')
				found = True
				break
			# If there is a match end->end, 
			# reverse the next curve
			p = curves[j].GetEndPoint(1)
			if DEBUG: print('END2END: {0}'.format(_sixteenth > p.DistanceTo(endPoint)))
			if _sixteenth > p.DistanceTo(endPoint):
				if i + 1 == j:
...
def sheetToViewTransform(sheetcoord):
	global transmatrix
	newx = transmatrix.destmin.X + (((sheetcoord.X -transmatrix.sourcemin.X)*(transmatrix.destmax.X - transmatrix.destmin.X))/(transmatrix.sourcemax.X - transmatrix.sourcemin.X))
	newy = transmatrix.destmin.Y + (((sheetcoord.Y -transmatrix.sourcemin.Y)*(transmatrix.destmax.Y - transmatrix.destmin.Y))/(transmatrix.sourcemax.Y - transmatrix.sourcemin.Y))
...
with Transaction(doc,'Set Crop Region') as t:
	...
	for bl in selboundary:
		newlinestart = sheetToViewTransform(bl.GeometryCurve.GetEndPoint(0))
		newlineend = sheetToViewTransform(bl.GeometryCurve.GetEndPoint(1))
		...
		if DEBUG: print('VP POLY LINE POINTS: {0}\n                     {1}\n'.format(
																						bl.GeometryCurve.GetEndPoint(0),
																						bl.GeometryCurve.GetEndPoint(1)
																						))

		if DEBUG: print('NEW CROP LINE POINTS: {0}\n                      {1}\n'.format(
																						newlinestart,
																						newlineend
																						))
