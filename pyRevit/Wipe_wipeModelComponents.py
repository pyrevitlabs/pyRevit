...
class purgeWindow:
	...
	def purgeProcess(self, sender, args):
		if doc.GetWorksharingCentralModelPath():
			...
			res = TaskDialog.Show('pyRevit', 'The central model is located at:\n {0}\n\nAre you sure you want to wipe?'.format(centralPath), TaskDialogCommonButtons.Yes | TaskDialogCommonButtons.Cancel)
...
def explodeAndRemoveAllGroups():
	...
	groups = list( FilteredElementCollector( doc ).OfCategory( BuiltInCategory.OST_IOSGroups ).ToElements() )
...
def removeAllSheets():
	...
	for s in sheets:
		if s.Id.IntegerValue in openViews:
			...
			try:
				report('{2}{0}  {1}'.format( s.LookupParameter('Sheet Number').AsString().rjust(10), s.LookupParameter('Sheet Name').AsString().ljust(50), s.Id ))
				doc.Delete( s.Id )
			except Exception as e:
