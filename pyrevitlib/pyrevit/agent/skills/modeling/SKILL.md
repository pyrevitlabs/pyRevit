...
- **Footprint roof:** a closed `CurveArray` footprint, then `doc.Create.NewFootPrintRoof(footprint, level, roof_type)`. From Python the out parameter comes back in a tuple: `roof, curves = ...`.
- **Gable roof:** one rectangular footprint. Call `roof.set_DefinesSlope(curve, True)` and `roof.set_SlopeAngle(curve, slope)` on the two eave edges only; the gable-end edges keep `DefinesSlope` False. Slope is rise over run, for example `0.5`.
...
- **Only when native elements can't express the geometry.**
...
- **Materials:** `FilteredElementCollector(doc).OfClass(DB.Material)`. Assign them through the element type's compound structure, or through material parameters.
