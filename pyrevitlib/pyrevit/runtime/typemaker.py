...
def create_avail_type(extension, cmd_component, module_builder=None):
    if cmd_component.type_id == exts.LINK_BUTTON_POSTFIX:
        mlogger.debug("Skipped creating availability type for: %s", cmd_component)
        return
    ...
    if module_builder:
        ...
        if context_str == exts.CTX_SELETION:
            bundletypemaker.create_selection_avail_type(
                module_builder=module_builder, cmd_component=cmd_component
            )
        ...
        mlogger.debug("Successfully created availability type for: %s", cmd_component)
    ...
    if module_builder:
        ...
        if cmd_component.type_id == exts.INVOKE_BUTTON_POSTFIX:
            invoketypemaker.create_executor_type(
                extension, module_builder, cmd_component
            )
        # if link
        elif cmd_component.type_id == exts.LINK_BUTTON_POSTFIX:
            # link buttons don't need types
            mlogger.debug("Skipped creating executor type for: %s", cmd_component)
        # if content
        ...
        # if url
            ...
            urltypemaker.create_executor_type(extension, module_builder, cmd_component)
...
def make_bundle_types(extension, cmd_component, module_builder=None):
    ...
    try:
        ...
        mlogger.error(
            "Error creating appropriate exec types for: %s | %s",
            cmd_component,
            createtype_err,
        )
    ...
    if cmd_component.context:
        try:
            ...
            mlogger.error(
                "Error creating appropriate avail types for: %s | %s",
                cmd_component,
                createtype_err,
            )
