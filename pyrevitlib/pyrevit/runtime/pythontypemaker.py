...
def _is_rocketmode_compat(ext_name):
    ...
    try:
        ...
        mlogger.error(
            "Error checking rocket-mode compatibility for extension: %s | %s",
            ext_name,
            ext_check_err,
        )
...
def create_executor_type(extension, module_builder, cmd_component):
    ...
    Args:
        ...
        cmd_component (pyrevit.extensions.genericcomps.GenericUICommand):
            command
    """
    cmd_component.requires_clean_engine = _does_need_clean_engine(
        extension, cmd_component
    )
