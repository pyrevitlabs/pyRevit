...
def _logobj_decoder(logobj):
    ...
    if 'revit' in logobj:
        ...
        # else it is not UsageRecord. It could be the command results
...
def _collect_records_from_file(usagelog_file):
    ...
    try:
        ...
        raise PyRevitException('Error reading usage log for: {} | {}'
                               .format(usagelog_file, err))
...
def _get_all_usagelogfiles(usagelog_path=None):
    # returns a list of all usage log files in the
...
def _collect_all_records(source_path=None):
    ...
    current_log_path = get_current_usage_logpath() \
...
def get_records(source_path=None, record_filter=None,
    ...
    """
    Returns a list of UsageRecord objects. Each UsageRecord object represents
    a usage log entry. if record_filter argument is provided with a
    RecordFilter (or any of the subclasses) object, this function will filter
    the records as well. Filter objects are provided in
    pyrevit.usagelog.filters module.

    Returned record list is sorterd using sorted() function.
    The UsageRecord object provides necessary methods to allow sorting
    ...
    Returns:
        list: returns a list of UsageRecord objects, sorted by date:time;
