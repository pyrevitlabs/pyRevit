...
REVIT_INI_PATH_FORMAT = "%APPDATA%/Autodesk/Revit/Autodesk Revit {version}/Revit.ini"
...
class CollabCacheModel(object):
    ...
    def __init__(self, model_path):
        ...
        self.central_cache_model_path = op.join(
            self.model_dir, "CentralCache", self.model_name_ex
        )
        self.model_backup_path = op.join(
            self.model_dir, "{}_backup".format(self.model_name)
        )
    ...
    def __str__(self):
        return "<{} id={} revit={}>".format(
            self.__class__.__name__, self.model_name_ex, self.product
        )
    ...
    def __init__(self, cache_path):
        ...
        for entry in os.listdir(self.cache_path):
            if entry.lower().endswith(".rvt"):
                self.cache_models.append(CollabCacheModel(op.join(cache_path, entry)))
...
def _get_custom_cache_root(version):
    ...
    ini_path = op.normpath(op.expandvars(REVIT_INI_PATH_FORMAT.format(version=version)))
    ...
    if not cfg.has_option(CLOUD_MODEL_CACHE_SECTION, CLOUD_MODEL_CACHE_KEY):
        mlogger.debug(
            "No %s key in [%s]", CLOUD_MODEL_CACHE_KEY, CLOUD_MODEL_CACHE_SECTION
        )
        return None
...
def _get_collab_cache_root(version):
    ...
    default = op.normpath(
        op.expandvars(COLLAB_CACHE_PATH_FORMAT.format(version=version))
    )
        ...
        for cache_root in os.listdir(collab_root):
            ...
            for cache_inst in os.listdir(cache_root_path):
                ...
                if op.isdir(cache_inst_path):
                    collab_caches.append(CollabCache(cache_inst_path))
...
def clear_model_cache(collab_cache_model):
    ...
    if isinstance(collab_cache_model, CollabCacheModel):
        ...
        try:
            ...
            mlogger.error(
                "Error deleting model cache @ %s | %s", cm.model_path, str(cmdel_ex)
            )
