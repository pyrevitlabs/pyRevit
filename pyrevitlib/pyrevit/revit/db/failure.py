...
RESOLUTION_TYPES = [
    DB.FailureResolutionType.MoveElements,
...
class FailureSwallower(DB.IFailuresPreprocessor):
    ...
    def _set_and_resolve(self, failuresAccessor, failure, res_type):
        ...
        try:
            ...
            mlogger.debug("error setting resolution: %s | %s", res_type, frex)
        ...
        if failure_reg:
            for failure_id in self._failures_swallowed:
                ...
                if failure_obj:
                    ...
                    mlogger.debug("can not find failure definition for: %s", failure_id)
        return failures
    ...
    def PreprocessFailures(self, failuresAccessor):
        ...
        if severity == coreutils.get_enum_none(DB.FailureSeverity):
            mlogger.debug(
                "clean document. returning withFailureProcessingResult.Continue"
            )
        ...
        for failure in failures:
            ...
            mlogger.debug(
                "\telements: %s",
                [get_elementid_value(x) for x in failure.GetFailingElementIds()],
            )
            ...
            if not failure_has_res and failure_severity == DB.FailureSeverity.Warning:
                failuresAccessor.DeleteWarning(failure)
                mlogger.debug(
                    "deleted warning with no acceptable resolution: %s", failure_guid
                )
                continue
            ...
            for res_type in RESOLUTION_TYPES:
                if default_res == res_type:
                    mlogger.debug("using default failure resolution: %s", res_type)
        ...
        if action_taken:
            mlogger.debug(
                "resolving failures with FailureProcessingResult.ProceedWithCommit"
            )
            return DB.FailureProcessingResult.ProceedWithCommit
        else:
            mlogger.debug("resolving failures with FailureProcessingResult.Continue")
            return DB.FailureProcessingResult.Continue
