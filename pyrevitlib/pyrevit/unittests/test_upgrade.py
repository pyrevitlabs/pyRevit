...
class HealBloatedTelemetryFieldsTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        try:
            ...
            raise unittest.SkipTest("Failed to load upgrade.py: {}".format(load_err))
    ...
    def test_preserves_valid_below_threshold_values(self):
        values = {
            ...
            "telemetry_server_url": json.dumps(
                "https://telem.example.com", separators=(",", ":"), ensure_ascii=False
            ),
            "apptelemetry_server_url": json.dumps(
                "https://apptelm.example.com", separators=(",", ":"), ensure_ascii=False
            ),
        }
        user_config, tempdir = self._make_config(values)
        ...
        self.assertEqual(
            values["telemetry_file_dir"],
            json.dumps("C:\\logs", separators=(",", ":"), ensure_ascii=False),
        )
        self.assertEqual(
