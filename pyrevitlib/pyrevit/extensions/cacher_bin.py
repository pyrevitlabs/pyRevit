...
def _get_cache_file(cached_ext):
    return appdata.get_data_file(
        file_id="cache_{}".format(cached_ext.name), file_ext="pickle"
    )
...
def get_cached_extension(installed_ext):
    ...
    try:
        ...
        raise PyRevitException(
            "Error reading cache for: {} | {}".format(installed_ext, err)
        )
...
def is_cache_valid(extension):
    try:
        ...
        mlogger.debug("Error reading cache file or file is not available: %s", err)
        return False
