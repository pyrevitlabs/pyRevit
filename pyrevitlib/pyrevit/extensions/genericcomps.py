...
class GenericUIComponent(GenericComponent):
    ...
    def make_unique_name(cls, cmp_path):
        ...
        return coreutils.cleanup_string(
            exts.UNIQUE_ID_SEPARATOR.join(pieces), skip=[exts.UNIQUE_ID_SEPARATOR]
        ).lower()
    ...
    def _update_from_directory(self):
        ...
        self.media_file = self.find_bundle_file(
            [exts.DEFAULT_MEDIA_FILENAME], finder="name"
        )
        mlogger.debug("Media file is: %s:%s", self.name, self.media_file)

        self._help_url = self.find_bundle_file([exts.HELP_FILE_PATTERN], finder="regex")
        ...
        if self.meta_file:
            ...
            try:
                ...
                mlogger.error("Error reading meta file @ %s | %s", self.meta_file, err)
    ...
    def _read_bundle_metadata(self):
        ...
        self._help_url = self.meta.get(exts.MDATA_COMMAND_HELP_URL, self._help_url)

        self.min_revit_ver = self.meta.get(
            exts.MDATA_MIN_REVIT_VERSION, self.min_revit_ver
        )
        self.max_revit_ver = self.meta.get(
            exts.MDATA_MAX_REVIT_VERSION, self.max_revit_ver
        )

        self.is_beta = self.meta.get(exts.MDATA_BETA_SCRIPT, "false").lower() == "true"

        highlight = self.meta.get(exts.MDATA_HIGHLIGHT_KEY, None)
        ...
        self.modules = self.meta.get(exts.MDATA_LINK_BUTTON_MODULES, self.modules)
    ...
    def configure(self, config_dict):
        configurable_params = ["_ui_title", "_tooltip", "_help_url", "author"]
...
class GenericUIContainer(GenericUIComponent):
    ...
    def _update_from_directory(self):
        ...
        if not self.parse_layout_metadata():
            mlogger.debug("Container does not have layout file defined: %s", self)
    ...
    def __iter__(self):
        ...
        if self.layout_items:
            ...
            for litem in self.layout_items:
                matching_cmp = next(
                    (x for x in self.components if x.name == litem.name), None
                )
                if matching_cmp:
                    # apply directives before adding to list
                    if litem.directive:
                        self._apply_layout_directive(litem.directive, matching_cmp)
            ...
            for idx, litem in enumerate(self.layout_items):
                if exts.SEPARATOR_IDENTIFIER in litem.name and idx < last_item_index:
                    separator = GenericUIComponent()
                    separator.type_id = exts.SEPARATOR_IDENTIFIER
                    laidout_cmps.insert(idx, separator)
                elif exts.SLIDEOUT_IDENTIFIER in litem.name and idx < last_item_index:
    ...
    def parse_layout_directive(self, layout_line):
        ...
        if parts:
            ...
            return source_item, LayoutDirective(
                directive_type=directive, target=target_value
            )
        return layout_line, None

    def parse_layout_item(self, layout_line):
        if layout_line:
            layout_item_name, layout_item_drctv = self.parse_layout_directive(
                layout_line
            )
            return LayoutItem(name=layout_item_name, directive=layout_item_drctv)
...
class GenericUICommand(GenericUIComponent):
    ...
    def _update_from_directory(self):
        ...
        if not self.config_script_file:
            mlogger.debug("Command %s: Does not have independent config script.", self)
    ...
    def _read_bundle_metadata(self):
        ...
        if exts.MDATA_ENGINE in self.meta:
            self.requires_clean_engine = (
                self.meta[exts.MDATA_ENGINE]
                .get(exts.MDATA_ENGINE_CLEAN, "false")
                .lower()
                == "true"
            )
            self.requires_fullframe_engine = (
                self.meta[exts.MDATA_ENGINE]
                .get(exts.MDATA_ENGINE_FULLFRAME, "false")
                .lower()
                == "true"
            )
            self.requires_persistent_engine = (
                self.meta[exts.MDATA_ENGINE]
                .get(exts.MDATA_ENGINE_PERSISTENT, "false")
                .lower()
                == "true"
            )
            ...
            rme = (
                self.meta[exts.MDATA_ENGINE].get(exts.MDATA_ENGINE_MAINTHREAD, "false")
                == "true"
            )
            # MDATA_ENGINE_DYNAMO_AUTOMATE is specific naming for dynamo
            automate = (
                self.meta[exts.MDATA_ENGINE].get(
                    exts.MDATA_ENGINE_DYNAMO_AUTOMATE, "false"
                )
                == "true"
            )
            ...
            self.dynamo_path = self.meta[exts.MDATA_ENGINE].get(
            ...
            self.dynamo_path_check_existing = (
                self.meta[exts.MDATA_ENGINE].get(
                    exts.MDATA_ENGINE_DYNAMO_PATH_CHECK_EXIST, "false"
                )
                == "true"
            )
            self.dynamo_force_manual_run = (
                self.meta[exts.MDATA_ENGINE].get(
                    exts.MDATA_ENGINE_DYNAMO_FORCE_MANUAL_RUN, "false"
                )
                == "true"
            )
            self.dynamo_model_nodes_info = self.meta[exts.MDATA_ENGINE].get(
        ...
        if self.type_id == exts.PANEL_PUSH_BUTTON_POSTFIX:
            ...
            self.context = self.meta.get(exts.MDATA_COMMAND_CONTEXT, None)
            if self.context:
    ...
    def _read_bundle_metadata_from_python_script(self):
        try:
            ...
            script_content = coreutils.ScriptFileParser(self.script_file)

            self._ui_title = (
                script_content.extract_param(exts.UI_TITLE_PARAM) or self._ui_title
            )

            script_docstring = script_content.get_docstring()
            custom_docstring = script_content.extract_param(exts.DOCSTRING_PARAM)
            self._tooltip = custom_docstring or script_docstring or self._tooltip
            ...
            self.is_beta = (
                script_content.extract_param(exts.BETA_SCRIPT_PARAM) or self.is_beta
            )
            ...
            self.requires_clean_engine = (
                script_content.extract_param(exts.CLEAN_ENGINE_SCRIPT_PARAM) or False
            )
            self.requires_fullframe_engine = (
                script_content.extract_param(exts.FULLFRAME_ENGINE_PARAM) or False
            )
            self.requires_persistent_engine = (
                script_content.extract_param(exts.PERSISTENT_ENGINE_PARAM) or False
            )
            ...
            if self.type_id == exts.PANEL_PUSH_BUTTON_POSTFIX:
                ...
                self.context = script_content.extract_param(exts.COMMAND_CONTEXT_PARAM)
                if self.context:
                    self.context = self._parse_context_directives(self.context)
    ...
    def script_language(self):
        if self.script_file is not None:
            ...
            ) or self.script_file.endswith(exts.GRASSHOPPERX_SCRIPT_FILE_FORMAT):
