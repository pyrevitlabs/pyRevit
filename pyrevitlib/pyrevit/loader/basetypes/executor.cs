...
namespace PyRevitBaseClasses
{
    ...
    public class ScriptExecutor
    {
        ...
        public ScriptExecutor(PyRevitCommand cmd,
        ...
        public int ExecuteScript(string sourcePath, string syspaths, string cmdName, string cmdUniqueName,
                                 ...
                                 ref Dictionary<String, String> resultDict)
        {
            try
            {
                ...
                // Setup engine (returns either an existing or a new engine)
                var engine = CreateEngine();

                // Create the scope and set __file__
                string scopeName = "__main__";
                var scope = CreateScope(engine, scopeName);
                ...
                scriptOutput.commandUniqueName = cmdUniqueName;
                ...
                compiler_options.ModuleName = scopeName;
        ...
        {
            return IronPython.Hosting.Python.CreateEngine(new Dictionary<string, object>()
            {{ "Frames", true },
             { "FullFrames", true },
        ...
        public ScriptScope CreateScope(ScriptEngine engine, string cmdUniqueName)
        {
            var scope = IronPython.Hosting.Python.CreateModule(engine, cmdUniqueName);
