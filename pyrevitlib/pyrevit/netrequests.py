    ...
    def __init__(self, dotnet_response, stream=False, client=None):
        ...
        try:
            ...
            try:
                ...
                try:
                    self._content = (
                        dotnet_response.Content.ReadAsByteArrayAsync().Result
                    )
                except Exception:
                    self._content = b""

                try:
                    self._text = dotnet_response.Content.ReadAsStringAsync().Result
                except Exception:
                    self._text = ""
    ...
    **kwargs
    ...
    try:
        ...
        if timeout is not None:
            try:
                client = HttpClient()
                client.Timeout = TimeSpan.FromSeconds(float(timeout))
