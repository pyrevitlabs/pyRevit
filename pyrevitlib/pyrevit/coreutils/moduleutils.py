...
def copy_func(func, func_name, doc_string=None, arg_list=None):
    ...
    new_func = types.FunctionType(
        func.func_code, func.func_globals, func_name, tuple(arg_list), func.func_closure
    )
...
def collect_marked(module_obj, prop_name):
    ...
    for member in inspect.getmembers(module_obj):
        _, type_obj = member
        if (inspect.isclass(type_obj) or inspect.isfunction(type_obj)) and getattr(
            type_obj, prop_name, False
        ):
