...
slogger = logger.get_logger(__name__)
...
def toggle_icon(new_state, on_icon_path=None, off_icon_path=None):
    ...
    uibutton = get_button()
    if not ibutton:
        slogger.debug('Can not find ui button.')
        return
    ...
    if not on_icon_path:
        ...
        if not on_icon_path:
            slogger.debug('Script does not have icon for on state.')
            return
    ...
    slogger.debug('Setting icon state to: {} ({})'.format(new_state, icon_path))
