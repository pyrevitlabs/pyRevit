...
def update_repo(repo_info):
    ...
    try:
        ...
        logger.debug(
            "Can not login to git repository to get updates: %s | %s",
            repo_info,
            auth_err,
        )
        raise auth_err
...
def check_for_updates():
    ...
    """
    now = time.time()
    last_check = envvars.get_pyrevit_env_var(envvars.CHECKUPDATES_TIMESTAMP_ENVVAR)
    ...
    if pyrevit_repo and get_updates(pyrevit_repo):
        ...
        for cmt_sha, cmt_msg in new_commits.items():
            ...
            if COREUPDATE_TRIGGER in cmt_msg:
                logger.debug("pyrevit repo has core update at %s: %s", cmt_sha, cmt_msg)
                return True
...
def update_pyrevit():
    ...
    if _check_connection():
        ...
        logger.debug("List of thirdparty repos to be updated: %s", thirdparty_repos)
        ...
        for repo_info in thirdparty_repos:
            ...
            try:
                ...
                logger.info(
                    ":inbox_tray: Successfully updated: %s to %s",
                ...
                logger.info(
                    "Can not update repo: %s (Run in debug to see why)", repo_info.name
                )
        ...
        if pyrevit_repo:
            if not pyrevit_has_coreupdates:
                logger.debug("Updating pyrevit repo: %s", pyrevit_repo.directory)
                try:
                    ...
                    logger.info(
                        "Can not update pyrevit repo (Run in debug to see why) | %s",
                        err,
                    )
