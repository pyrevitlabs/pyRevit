...
class RevitServer(object):
    ...
    def __repr__(self):
        ...
        Example:
            ...
            <rpws.RevitServer name:server01 version:2017>
        """
        return "<rpws.{} name:{} version:{}>".format(
    ...
    def _header_dict(self):
        ...
        return {
            api.REQ_HEADER_USERNAME: self._huser,
    ...
    def _httpmethod(self, http_method, command, node_uri=None, rootcmd=False):
        ...
        if rootcmd:
            ...
            req_url = (
                self._base_uri
                + "/"
                + urllib.quote(self._api_path(node_uri).encode("utf-8"))
                + command
            )
        ...
        if r.status_code == 200 and r.text:
            ...
            raise rpws.ServerNotImplemented(
                "name:{} version:{}".format(self.name, self.version)
            )
        elif r.status_code == 503:
            raise rpws.ServerServiceUnavailableError(node_uri)
        else:
            raise rpws.UnhandledException(
                "requests status code:{}".format(r.status_code)
            )
    ...
    def _getlocks(ip_lock_list):
        ...
        if ip_lock_list:
            ...
            for ip_lock in ip_lock_list:
                ...
                ts = models.DateEntry.fromrsdatestring(
                    ip_lock[api.NODE_LIP_TIMESTAMP_KEY]
                )

                # extract and create timespan obj
                tspan = models.TimeSpanEntry.fromrstimespanstring(
                    ip_lock[api.NODE_LIP_AGE_KEY]
                )
    ...
    def _getfiles(self, nodepath, contents_dict):
        ...
        return [
            models.FileInfo(
                path=op.join(
                    nodepath if nodepath else self.path, x[api.NODE_FILES_NAME_KEY]
                ),
    ...
    def _getmodels(self, nodepath, contents_dict):
        ...
        for mdict in contents_dict.get(api.NODE_MODELS_KEY, []):
            ...
            minfo = models.ModelInfo(
                path=op.join(
                    nodepath if nodepath else self.path, mdict[api.NODE_MODELS_NAME_KEY]
                ),
    ...
    def getinfo(self):
        ...
        # server roles
        sroles = [models.ServerRole(x) for x in props_dict[api.SERVER_ROLES_KEY]]
    ...
    def listfiles(self, nodepath=None):
        ...
        Example:
            ...
            ...     print(file.path)
        """
        return self._getfiles(nodepath, self._get(api.REQ_CMD_CONTENTS, nodepath))
    ...
    def listfolders(self, nodepath=None):
        ...
        Example:
            ...
            ...     print(folder.path)
        """
        return self._getfolders(nodepath, self._get(api.REQ_CMD_CONTENTS, nodepath))
    ...
    def listmodels(self, nodepath=None):
        ...
        Example:
            ...
            ...     print(model.path)
        """
        return self._getmodels(nodepath, self._get(api.REQ_CMD_CONTENTS, nodepath))
    ...
    def getfolderinfo(self, nodepath):
        ...
        if nodepath:
            ...
            # in-progress locks
            ip_locks = self._getlocks(ddict[api.NODE_DIRINFO_LOCKSINPROGRESS_KEY])

            # make lock state
            lock_state = models.LockState(ddict[api.NODE_DIRINFO_LOCKSTATE_KEY])

            # make time stamps
            date_created = models.DateEntry.fromrsdatestring(
                ddict[api.NODE_DIRINFO_DATECREATED_KEY]
            )
            date_modified = models.DateEntry.fromrsdatestring(
                ddict[api.NODE_DIRINFO_DATEMODIFIED_KEY]
            )
    ...
    def getmodelinfo(self, nodepath):
        ...
        if nodepath:
            ...
            # make time stamps
            dc = models.DateEntry.fromrsdatestring(
                mdict[api.NODE_MODELINFO_DATECREATED_KEY]
            )
            dm = models.DateEntry.fromrsdatestring(
                mdict[api.NODE_MODELINFO_DATEMODIFIED_KEY]
            )
    ...
    def getmodelhistory(self, nodepath):
        ...
        for hitem_dict in mhist_dict[api.MHISTORY_ITEMS_KEY]:
            # make time stamp
            mhist_date = models.DateEntry.fromrsdatestring(
                hitem_dict[api.MHISTORY_DATE_KEY]
            )
        ...
        return models.MHistoryInfo(
            path=mhist_dict[api.MHISTORY_PATH_KEY], items=hist_items
        )
    ...
    def rename(self, nodepath, new_nodename):
        ...
        """
        return self._delete(api.REQ_CMD_RENAME.format(new_name=new_nodename), nodepath)
    ...
    def copy(self, nodepath, new_nodepath, overwrite=False):
        ...
        return self._post(
            api.REQ_CMD_COPY.format(dest_path=new_apipath, replace_exist=overwrite),
            nodepath,
        )
    ...
    def move(self, nodepath, new_nodepath, overwrite=False):
        ...
        new_apipath = self._api_path(new_nodepath)
        return self._post(
            api.REQ_CMD_MOVE.format(dest_path=new_apipath, replace_exist=overwrite),
            nodepath,
        )
    ...
    def walk(self, top=None, topdown=True, digmodels=False):
        ...
        if digmodels:
            for minfo in entry_info.models:
                for x in self.walk(
                    top=minfo.path, topdown=topdown, digmodels=digmodels
                ):
                    yield x
        if not topdown:
            # Yield before recursion if going top down
