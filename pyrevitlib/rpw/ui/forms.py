...
from rpw import HOST_APP, UI, get_logger, PYREVIT_CMDPATH
...
class WPFWindow(Window):
    def __init__(self, xaml_file, literal_string=False):
        ...
        if not literal_string:
            if not op.exists(xaml_file) and PYREVIT_CMDPATH:
                # noinspection PyUnresolvedReferences
                wpf.LoadComponent(self,
                                  os.path.join(PYREVIT_CMDPATH, xaml_file))
            else:
    ...
    def set_image_source(self, element_name, image_file):
        ...
        if not op.exists(image_file):
            # noinspection PyUnresolvedReferences
            wpf_element.Source = \
                BitmapImage(Uri(os.path.join(PYREVIT_CMDPATH, image_file)))
        else:
            wpf_element.Source = \
                BitmapImage(Uri(image_file))
...
class TemplatePromptBar(WPFWindow):
    ...
    <Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
            ...
            WindowStyle="None" Background="{x:Null}" ShowInTaskbar="False"
            ShowActivated="False"
