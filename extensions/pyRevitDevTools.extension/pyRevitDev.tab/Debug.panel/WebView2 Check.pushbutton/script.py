...
WV2_USERDATA = os.path.join(os.environ.get("LOCALAPPDATA", ""), "pyRevit", "WebView2")
...
def t_inject_head():
    ...
    output.inject_to_head("style", "#wv2check{color:purple;}", {"id": "wv2check-style"})
...
def t_add_style():
    ...
    output.add_style(
        ".wv2check-entry{letter-spacing:0.5px;}", {"id": "wv2check-style2"}
    )
...
def t_markdown_link():
    ...
    output.print_md(
        "Markdown link check: [pyRevit Repo](https://github.com/pyrevitlabs/pyRevit)"
    )
    r = output.evaluate_js(
        "!!document.querySelector(\"a[href='https://github.com/pyrevitlabs/pyRevit']\")"
    )
...
def t_legacy_renderer_removed():
    ...
    try:
        ...
        return (False, "output.renderer raised unexpected {}".format(type(ex).__name__))
...
def t_emoji_unicode():
    ...
    r = output.evaluate_js(
        'document.getElementById("wv2check-uni").innerText.indexOf("✅") >= 0'
    )
...
def t_emoji_segoe():
    ...
    output.print_html(
        '<span id="wv2check-segoe" style="font-family:\'Segoe UI Emoji\'">✅</span>'
    )
    r = output.evaluate_js(
...
def t_emoji_dll_pipeline():
    ...
    output.print_html(
        '<div id="wv2check-dll">:heavy_check_mark: :cross_mark: :warning:</div>'
    )
