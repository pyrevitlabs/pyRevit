"""Run all config unit tests from pyrevit.unittests."""
...
config_test_modules = _discover_config_modules()
if not config_test_modules:
    raise RuntimeError(
        "No config unit test modules found with prefix '{}' in {}".format(
            TEST_MODULE_PREFIX,
            tests_pkg.__name__,
        )
    )
