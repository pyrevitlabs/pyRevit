    ...
    def filter_entries(self, entry_list, search_term=None):
        ...
        for entry in entry_list:
            if getattr(entry, self.type_id) == self.filter_value:
                if search_term:
                    if (
                        search_term.lower() in entry.message.lower()
...
class LogMessageListItem(object):
    def __init__(self, log_entry):
        ...
        self.date, self.time, self.level, self.module, self.message = (
            log_entry_parser.findall(self._log_entry)[0]
        )
        try:
...
class LogViewerWindow(forms.WPFWindow):
    ...
    def _read_log_file(self, file_path):
        ...
        try:
            with open(file_path, "r") as log_file:
                for log_entry in log_file.readlines():
                    try:
                        ...
                        slogger.debug(
                            "Error processing entry at {}:{}".format(
            ...
            slogger.error("Error reading log file: {} | {}".format(file_path, read_err))
    ...
    def log_file_changed(self, sender, args):
        self._current_entry_list = self._read_log_file(
            self._log_files[self.current_log_file]
        )
    ...
    def filter_changed(self, sender, args):
        ...
        if cur_filter:
            filtered_list = cur_filter.filter_entries(
                self._current_entry_list, search_term=self.search_tb.Text
            )
