...
def load_pgks_from_file():
    ...
    extensions.json should be get_formatted like this:
    ...
    """
    logger.info('Getting Local Extension list...')
    ...
    try:
        ...
        logger.error('Packages is not properly get_formatted.')
        logger.error(errmsg)
        raise
...
def load_pgks_from_origin():
    ...
    logger.info('Getting Remote Extension list...')
    try:
...
def clone_pkg_from_remote(package_name, package_url):
    ...
    if not package_exists:
        logger.debug('Extension does not exists. Will Clone.')
        logger.info('Clonning Extension from Repository...')
        process, out, errmsg = subprocess_cmd(
        ...
        logger.debug('Extension Exists. Will merge')
        logger.info('Updating Extension from Repository...')
...
def remove_local_pkg(package_name):
    ...
    logger.info('Deletting Extension...')
    try:
        ...
        logger.error(errmsg)
        TaskDialog.Show('Extension Maanger', 'Could not Delete Extension.'\
                        'Please manually delete:\n{}'.format(
                         os.path.join(CWD, package_name)))
    else:
        logger.info('Extension Deleted: %s', package_name)
