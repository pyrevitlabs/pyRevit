...
selected_switch = forms.CommandSwitchWindow.show(
    sorted(switches), message="List elements of type:"
)
...
if selected_switch == "Graphic Styles":
    ...
    for gs in gstyles:
        ...
        if gs.GraphicsStyleCategory.GetHashCode() > 0:
            print(
                "NAME: {0} CATEGORY:{2} PARENT: {3} ID: {1}".format(
                    gs.Name.ljust(50),
                    gs.Id,
    ...
    cl = DB.FilteredElementCollector(revit.doc)
    grid_list = (
        cl.OfCategory(DB.BuiltInCategory.OST_Grids)
    ...
    for lineStyle in revit.query.get_line_styles(doc=revit.doc):
        print(
            "STYLE NAME: {} ID: {} ({})".format(
                lineStyle.Name.ljust(40), lineStyle.Id.ToString(), lineStyle
            )
        )
    ...
    for c in cllines:
        print(
            "{0:<10} {1:<25}{2:<8} {3:<15} {4:<20}".format(
                c.Id,
                c.GetType().Name,
                c.LineStyle.Id,
                c.LineStyle.Name,
    ...
    project_params = set(
        [
            x.name + ":" + x.param_guid
            for x in revit.query.get_project_parameters()
            if x.shared
        ]
    )
    ...
    all_types = revit.query.get_types_by_class(DB.ElementType, doc=revit.doc)
    ...
    for etype_name in sorted(etypes_dict.keys()):
        ...
        for et in etypes:
            print("\t{} {}".format(output.linkify(et.Id), revit.query.get_name(et)))
    ...
    for i in levelslist:
        print(
            "Level ID:\t{1}\tELEVATION:{2}\t\tName:\t{0}".format(
                i.Name, get_elementid_value(i.Id), i.Elevation
            )
        )
    ...
    for el in arealist:
        print(
            "AREA NAME: {0} AREA NUMBER: {1} AREA ID: {2} LEVEL: {3} AREA: {4}".format(
                el.Parameter[DB.BuiltInParameter.ROOM_NAME].AsString().ljust(30),
    ...
    for el in roomlist:
        print(
            "ROOM NAME: {0} ROOM NUMBER: {1} ROOM ID: {2}".format(
                el.Parameter[DB.BuiltInParameter.ROOM_NAME].AsString().ljust(30),
    ...
    cl = DB.FilteredElementCollector(revit.doc)
    revs = cl.OfCategory(
        DB.BuiltInCategory.OST_Revisions
    ).WhereElementIsNotElementType()
    ...
    sheetsnotsorted = (
        cl_sheets.OfCategory(DB.BuiltInCategory.OST_Sheets)
        .WhereElementIsNotElementType()
        .ToElements()
    )
    ...
    for s in sheets:
        print(
            "NUMBER: {0}   NAME:{1}".format(
                s.Parameter[DB.BuiltInParameter.SHEET_NUMBER].AsString().rjust(10),
    ...
    for pc in UI.PostableCommand.GetValues(UI.PostableCommand):
        try:
            ...
            if rcid:
                print(
                    "{0} {1} {2}".format(
                        str(pc).ljust(50), str(rcid.Name).ljust(70), rcid.Id
                    )
                )
            else:
    ...
    views = (
        cl_views.OfCategory(DB.BuiltInCategory.OST_Views)
        .WhereElementIsNotElementType()
        .ToElements()
    )
    ...
    for v in views:
        if v.IsTemplate:
            print("{0} {1}".format(revit.query.get_name(v), output.linkify(v.Id)))
    ...
    for rev in revs:
        ...
        if isinstance(parent, DB.ViewSheet):
            print(
                "{0}\t\tID: {3}\t\tON SHEET: {1} {2}".format(
                    revnumstr, parent.SheetNumber, parent.Name, rev.Id
                )
            )
        else:
            print(
                "{0}\t\tID: {2}\t\tON VIEW: {1}".format(
                    revnumstr, revit.query.get_name(parent), rev.Id
                )
            )
    ...
    schemas = {x.GUID.ToString(): x for x in revit.query.get_all_schemas()}
    ...
    for el in revit.query.get_all_elements(doc=revit.doc):
        ...
        for guid_obj in schema_guids:
            ...
            if guid in schemas:
                ...
                for fname, fval in revit.query.get_schema_field_values(
                    el, schema
                ).items():
    ...
    for el in selection:
        if isinstance(el, DB.FilledRegion):
            ...
            print("\n\n Filled Region Type: {}".format(revit.query.get_name(frt)))
    ...
    for esvc in DB.ExternalService.ExternalServiceRegistry.GetServices():
        ...
        for sid in esvc.GetRegisteredServerIds():
            ...
            print(
                "{} | {} ({}) (Builtin: {})".format(
                    sid, server.GetName(), server.GetDescription(), sid in bisrvids
                )
            )
