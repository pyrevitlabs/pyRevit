...
class AboutWindow(forms.WPFWindow):
    def __init__(self, xaml_file_name):
        ...
        short_version = " v{}".format(pyrvt_ver.get_formatted(strict=True))
        ...
        if cpyver:
            self.pyrevit_engine.Text = self.get_locale_string("pyRevit.Engine").format(
                sys.version.split("(")[0].strip(), cpyver.Version
            )

        if __cachedengine__:
            self.pyrevit_rmode.Text = self.get_locale_string(
                "pyRevit.RocketMode.Enabled"
            )
        else:
            self.pyrevit_rmode.Text = self.get_locale_string(
                "pyRevit.RocketMode.Disabled"
            )
    ...
    def opengithubcommits(self, sender, args):
        if self.branch_name:
            commits_url = urls.PYREVIT_GITHUBBRANCH_COMMIT.format(
    ...
    def opengithubbranch(self, sender, args):
        if self.branch_name:
            branch_url = urls.PYREVIT_GITHUBBRANCH.format(branch=self.branch_name)
