...
Every hook script in this extension logs through :func:`log_hook`, so the
host-handle fields recorded there apply to all of them at once.
"""
...
def log_hook(hook_file, data, log_doc_access=False):
    ...
    Every record carries two fields that pin down pyRevit's host-handle
    contract: ``handle`` is the type of the ``__revit__`` builtin, which must
    read ``UIApplication`` in every hook context, and ``sender`` is the type of
    the raw event sender, which is expected to vary by event and proves the
    sender survived normalization as ``__eventsender__``.
