...
class CommitedSheet(object):
    ...
    def _read_commit_history(self, commit_cfg):
        ...
        for commit_pt in [
            x
            for x in self.commit_history.commit_points
            ...
            if pkg_param:
                ...
                if ctype:
                    try:
                        self.commit(commit_pt, commit_type=ctype)
        ...
        sheet_revids = {
            get_elementid_value(x) for x in self.revit_sheet.GetAllRevisionIds()
        }
        add_sheet_revids = {
            get_elementid_value(x) for x in self.revit_sheet.GetAdditionalRevisionIds()
        }
                ...
                if commit_pt.target == sheet_revid:
                    ...
                    try:
                        new_commit = self.commit(
                            commit_pt, commit_type=default_rev_commit_type
                        )
    ...
    def update_commit_history(self):
        ...
        for commit_pt in [
            ...
            if pkg_param:
                ...
                mlogger.error(
                    'Package parameter "%s" does not exist for %s:%s',
                    commit_pt.target,
                    self.number,
                    self.name,
                )
        ...
        for commit_pt in [
            ...
            if commit.commit_type == CommitTypes.Revised and not commit.read_only:
                sheet_revs.append(revision)
        revit.update.update_sheet_revisions(all_revs, [self.revit_sheet], state=False)
        revit.update.update_sheet_revisions(sheet_revs, [self.revit_sheet])

    def can_commit(self, commit_point, commit_type, allow_endpoint_change=False):
        return self.commit_history.can_commit_at_point(
            commit_point, commit_type, allow_endpoint_change=allow_endpoint_change
        )
...
def get_commit_points():
    ...
    dockpkgs = []
    get_elementid_value = get_elementid_value_func()
    docpkg_finder = re.compile(r"docpkg(\d+)\s*[-_]*?\s*(.+)", flags=re.IGNORECASE)
    for project_param in revit.query.get_project_parameters(doc=revit.doc):
        ...
        if pkg_match:
            ...
            mlogger.warning(
                'Package parameter "%s" is not formatted correctly and is skipped.',
                project_param.name,
            )
    last_docpkg_idx = -1
    for idx, docpkg in enumerate(sorted(dockpkgs, key=lambda x: x.pkg_idx)):
        ...
        commit_points.append(
            CommitPoint(
                cptype=CommitPointTypes.Package,
    ...
    docrevs = sorted(revit.query.get_revisions(), key=lambda x: x.SequenceNumber)
    commit_points.extend(
        [
            CommitPoint(
                cptype=CommitPointTypes.Revision,
                ...
                desc="{}{} (Sequence #{})".format(
                    ...
                    if revit.ensure.revision_has_numbertype(x)
                    else "",
                    x.Description,
                    x.SequenceNumber,
                ),
