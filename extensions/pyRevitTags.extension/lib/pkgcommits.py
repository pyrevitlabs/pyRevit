...
class CommitTypes(object):
    ...
    Merged = CommitType(order=6, idx="merged", name="Merged", ends_history=True)

    Deleted = CommitType(order=7, idx="deleted", name="Deleted", ends_history=True)

    @staticmethod
    def get_types():
        return [
            getattr(CommitTypes, x)
...
class CommitPointType(object):
    def __init__(
        ...
        if self.requires_history_start:
            self.allowed_history_start_commit_types = allowed_history_start_commit_types
            self.allowed_history_end_commit_types = allowed_history_end_commit_types
...
class CommitPointTypes(object):
    Package = CommitPointType(
        ...
        allowed_history_start_commit_types=[CommitTypes.Created, CommitTypes.IssuedRe],
        allowed_history_end_commit_types=[CommitTypes.Deleted, CommitTypes.Merged],
    )
    ...
    def get_targets():
        return [
            getattr(CommitPointTypes, x)
            for x in dir(CommitPointTypes)
...
class CommitPoint(object):
    ...
    def __repr__(self):
        return "<{} cptype={} idx={} name={}>".format(
            self.__class__.__name__, self.cptype, self.idx, self.name
        )
...
class Commit(object):
    ...
    @classmethod
    def from_point(
        cls, commit_pt, commit_source=CommitSources.User, commit_type=CommitTypes.NotSet
    ):
        return cls(
            commit_point=commit_pt, commit_source=commit_source, commit_type=commit_type
        )

    def __hash__(self):
        return hash(
            str(self.commit_point) + str(self.commit_type) + str(self.commit_source)
        )
...
class CommitHistory(object):
    ...
    def __repr__(self):
        history_graph = "-----".join(
            ["[{}:{}]".format(x.commit_point.idx, x.commit_type) for x in self.commits]
        )
        return "<{} graph={} commits={}>".format(
            self.__class__.__name__, history_graph, self.commits
        )

    def _find_starting_commits(self):
        return sorted(
            [x for x in self.commits if x.commit_type.starts_history],
            key=lambda x: x.commit_point.idx,
        )

    def _find_ending_commits(self):
        return sorted(
            [x for x in self.commits if x.commit_type.ends_history],
            key=lambda x: x.commit_point.idx,
        )
        ...
        for commit_point in self.commit_points:
            ...
            if commit:
                ...
                if commit.commit_type.requires_after:
                    ...
                    mlogger.debug(
                        "Commit %s requires after: %s", commit, next_commit_type
                    )
                    continue
            ...
            if (
                (next_commit_type and commit_source)
                and not commit
                and commit_point.cptype.requires_history_start
            ):
                mlogger.debug(
                    "Applying non-user(%s) commit type: %s",
                    commit_source,
                    next_commit_type,
                )
    ...
    def _can_commit(self, commit):
        ...
        user_commits_before = [
            x for x in commits_before if x.commit_source == CommitSources.User
        ]
        mlogger.debug("User commits before: %s", user_commits_before)
        hist_ended_before = any(
            [x.commit_type.ends_history for x in user_commits_before]
        )
        hist_started_before = any(
            [x.commit_type.starts_history for x in user_commits_before]
        )
        ...
        if user_commits_before:
            ...
            if commit_type.starts_history and hist_ended_before:
                mlogger.debug(
                    "Insert start and history has ended before. "
            ...
            if commit_type.starts_history and hist_started_before:
                mlogger.debug(
                    "Insert start and history has started or is "
            ...
            if commit_type.ends_history and hist_ended_before:
                mlogger.debug(
                    "Insert end and history has ended before. "
                    "raising HistoryEndedBefore"
                )
                raise HistoryEndedBefore()
            ...
            if commit_type.ends_history and hist_started_before:
                mlogger.debug(
                    "Insert end and history has started or is "
                    "recurring before. skipping..."
                )

            if (
                not (commit_type.starts_history or commit_type.ends_history)
                and hist_ended_before
            ):
                mlogger.debug(
                    "Insert non-starting non-ending "
        ...
        user_commits_after = [
            x for x in commits_after if x.commit_source == CommitSources.User
        ]
        mlogger.debug("User commits after: %s", user_commits_after)
        hist_started_after = any(
            [x.commit_type.starts_history for x in user_commits_after]
        )
        hist_ended_after = any([x.commit_type.ends_history for x in user_commits_after])
        ...
        if user_commits_after:
            ...
            if commit_type.starts_history and hist_started_after:
                mlogger.debug(
                    "Insert start and history has started or "
            ...
            if commit_type.ends_history and hist_ended_after:
                mlogger.debug(
                    "Insert end and history has ended after. raising HistoryEndedAfter"
                )
        ...
        if not commit_point.cptype.requires_history_start:
            mlogger.debug(
                "Insert into commit point type that "
                "does not require a start. returning ok to insert"
            )
            return True

        if (
            commit_point.cptype.requires_history_start
            and commit_type.ends_history
            and hist_started_after
        ):
            mlogger.debug(
                "Insert as first ending into commit point "
        ...
        if (
            ...
            mlogger.debug(
                "Insert as first non-starting into commit point "
                "type that requires a start commit. "
    ...
    def get_commit_before(self, commit_point):
        ...
        if prev_commits:
            prev_commit = sorted(prev_commits, key=lambda x: x.commit_point.idx)[-1]
            mlogger.debug("Commit %s found at point %s", prev_commit, commit_point)
            return prev_commit
    ...
    def get_commit_after(self, commit_point):
        ...
        if next_commits:
            next_commit = sorted(next_commits, key=lambda x: x.commit_point.idx)[0]
            mlogger.debug("Commit %s found at point %s", next_commit, commit_point)
            return next_commit
    ...
    def get_commit_at(self, commit_point):
        ...
        for commit in self.commits:
            if commit.is_at(commit_point):
                mlogger.debug("Commit %s found at point %s", commit, commit_point)
                return commit
    ...
    def can_commit_at_point(
        self, commit_point, commit_type, allow_endpoint_change=False
    ):
        try:
    ...
    def commit_at_point(
        ...
        # notset commit?
            ...
            new_commit = Commit.from_point(
                commit_point, commit_source=commit_source, commit_type=commit_type
            )
            try:
