...
class ManageTagsWindow(forms.WPFWindow):
    def __init__(self, xaml_file_name):
        ...
        if sel_cnt:
            forms.alert(
                "You are managing tags on {} selected elements only.".format(sel_cnt)
            )
        ...
        if sel_cnt > 0:
            ...
            self._context = tagsmgr.get_available_tags(self._target_elements)
        else:
        ...
        self.tagmodif_template = self.Resources["ModifierListItemControlTemplate"]
        self._list_options()
        ...
        if self.selected_tags:
            ...
            try:
                with revit.Transaction("Delete Tag"):
                    tagsmgr.remove_tag(self.selected_tags[0], self._target_elements)
            ...
            if new_tag:
                try:
                    with revit.Transaction("Rename Tag", log_errors=False):
                        tagsmgr.rename_tag(
                            self.selected_tags[0], new_tag, self._target_elements
                        )
                except Exception as e:
                    forms.alert(getattr(e, "msg", str(e)))
    ...
    def add_modifier(self, sender, args):
        if self.selected_tags:
            ...
            modifiers = forms.SelectFromList.show(
            ...
            if modifiers:
                self.Close()
                try:
                    with revit.Transaction("Apply Tag Modifier", log_errors=False):
                        tagsmgr.add_modifier(
                            self.selected_tags, modifiers, self._target_elements
                        )
                except Exception as e:
                    forms.alert(getattr(e, "msg", str(e)))
    ...
    def remove_modifier(self, sender, args):
        if self.selected_tags:
            ...
            if modifiers:
                self.Close()
                try:
                    with revit.Transaction("Remove Tag Modifier", log_errors=False):
                        tagsmgr.remove_modifier(
                            self.selected_tags, modifiers, self._target_elements
                        )
                except Exception as e:
                    forms.alert(getattr(e, "msg", str(e)))
    ...
    def make_filter(self, sender, args):
        if self.selected_tags:
            self.Close()
            try:
                with revit.Transaction("Create Tag Filters", log_errors=False):
    ...
    def make_modifier_filters(self, sender, args):
        self.Close()
        try:
            with revit.Transaction("Create Modifier Filters", log_errors=False):
    ...
    def make_schedule(self, sender, args):
        ...
        if tag:
            ...
            tag_cat = forms.SelectFromList.show(
            ...
            if tag_cat:
                ...
                if tag_sched:
                    ...
                    with revit.Transaction(
                        "Create Tag Schedule ({})".format(tag_cat.Name)
                    ):
                        new_Sched = tagsmgr.create_tag_schedule(tag, tag_cat, tag_sched)
        ...
        if self.selected_tags:
            ...
            try:
                with revit.Transaction("Create Tag 3D View", log_errors=False):
                    tag_view = tagsmgr.create_tag_3dview(self.selected_tags)
