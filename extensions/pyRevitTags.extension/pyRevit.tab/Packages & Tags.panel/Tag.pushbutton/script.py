...
class ApplyTagWindow(forms.WPFWindow):
    def __init__(self, xaml_file_name):
        ...
        self._tag_cache = script.get_document_data_file(
            file_id="tagcache", file_ext="cache"
        )
    ...
    def _apply_tags(self, title="Apply Tags", append=True):
        self.Close()
        cfg = tagsmgr.ApplyTagsConfig(
            append=append, circuits=self.circuits_cb.IsChecked
        )
