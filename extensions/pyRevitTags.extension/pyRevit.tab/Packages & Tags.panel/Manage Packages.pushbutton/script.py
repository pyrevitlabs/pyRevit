...
class CommitItem(object):
    ...
    @property
    def passthru(self):
        return self.hasbefore and self.hasafter and self.ctype == CommitTypes.NotSet
...
class SheetItem(object):
    ...
    def _get_commit_item(self, commit_pt):
        for commit in self._csheet.commit_history:
            if commit.is_at(commit_pt):
                prev_commit = self._csheet.commit_history.get_prev_commit(commit)
                next_commit = self._csheet.commit_history.get_next_commit(commit)
                return CommitItem(
                    commit,
    ...
    def __getattr__(self, attr_name):
        ...
        if pkgmgr.CommitPointTypes.Package.idx in attr_name:
            cidx = int(attr_name.replace(pkgmgr.CommitPointTypes.Package.idx, ""))
        elif pkgmgr.CommitPointTypes.Revision.idx in attr_name:
            cidx = int(attr_name.replace(pkgmgr.CommitPointTypes.Revision.idx, ""))
    ...
    def can_commit(self, commit_point, commit_type):
        return self._csheet.can_commit(
            commit_point, commit_type, allow_endpoint_change=True
        )

    def commit(self, commit_point, commit_type):
        self._csheet.commit(commit_point, commit_type, allow_endpoint_change=True)
        self._update_commit_items()
...
class ManagePackagesWindow(forms.WPFWindow):
    def __init__(self, xaml_file_name):
        ...
        if sel_cnt:
            forms.alert(
                "You are managing packages on {} selected sheets only.".format(sel_cnt)
            )
        ...
        self.column_header_template = self.sheets_dg.Resources["ColumnHeaderTemplate"]
        self.package_column_cell_style = self.sheets_dg.Resources["PackageCellStyle"]
        self.revision_column_cell_style = self.sheets_dg.Resources["RevisionCellStyle"]
        self.committype_template = self.Resources["CommitTypeListItemControlTemplate"]
        ...
        sheet_numbers = [
            x.SheetNumber for x in selection if isinstance(x, DB.ViewSheet)
        ]

        if sheet_numbers:
            sheetitems = [
                SheetItem(x) for x in committed_sheets if x.number in sheet_numbers
            ]
        else:
    ...
    def _list_sheets(self, sheet_filter=None):
        ...
        if sheet_filter:
            ...
            items_source = [
                x
                for x in items_source
                if sheet_filter in x.number.lower() or sheet_filter in x.name.lower()
            ]
        ...
        if self.sheetorder_param:
            ...
            items_source = sorted(items_source, key=lambda x: x.get_order(param))
        ...
        if self.sheets_dg.ItemsSource:
            self.sheets_dg.ScrollIntoView(
                self.sheets_dg.ItemsSource[0], list(self.sheets_dg.Columns)[-1]
            )
    ...
    def _ask_for_commit_type(self, sheet_item, commit_pt, commit):
        ...
        if allowed_types:
            ctype_commit_op = forms.SelectFromList.show(
                sorted(
                    [CommitTypeOption(x) for x in allowed_types],
    ...
    def _setup_item_params_combobox(self, committed_sheets):
        if committed_sheets:
            ...
            sheet_params = [
                x.Definition.Name
                for x in csheet.revit_sheet.Parameters
    ...
    def edit_commit(self, sender, args):
        if self.sheets_dg.CurrentCell:
            ...
            commit_type = self._ask_for_commit_type(sheet_item, commit_pt, commit)
            if commit_type:
                try:
                    sheet_item.commit(commit_pt, commit_type)
                except pkgexceptions.HistoryStartMustBeFirst:
                    forms.alert(
                        'First change must be a "Create" or "Issued for Reference".'
                    )
