...
def who_reloaded_keynotes():
    ...
    if revit.doc.IsWorkshared:
        try:
            ...
            forms.alert("Model is not saved yet. Can not aquire keynote file location.")
    else:
...
def who_created_selection():
    ...
    if revit.doc.IsWorkshared:
        if selection and len(selection) == 1:
            ...
            forms.alert(
                "Creator: {0}\nCurrent Owner: {1}\nLast Changed By: {2}".format(
                    eh.creator, eh.owner, eh.last_changed_by
                )
            )
        else:
...
def who_created_activeview():
    ...
    view_creator = DB.WorksharingUtils.GetWorksharingTooltipInfo(
        revit.doc, active_view.Id
    ).Creator

    forms.alert(
        "{}\n{}\n{}".format(
            "Creator of the current view: " + view_name,
            "with the id: " + view_id,
            "is: " + view_creator,
        )
    )
...
options = {
    "Who Created Active View?": who_created_activeview,
    "Who Created Selected Element?": who_created_selection,
    "Who Reloaded Keynotes Last?": who_reloaded_keynotes,
}

selected_option = forms.CommandSwitchWindow.show(options.keys())
