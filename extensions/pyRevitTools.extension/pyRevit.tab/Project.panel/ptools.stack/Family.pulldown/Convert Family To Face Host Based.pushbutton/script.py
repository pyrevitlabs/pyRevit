...
def delete_all_instances(target_family):
    ...
    try:
        ...
        for symbol_id in symbol_ids:
            for family_instance in (
                DB.FilteredElementCollector(revit.doc)
                .WherePasses(DB.FamilyInstanceFilter(revit.doc, symbol_id))
                .ToElements()
            ):
...
if forms.alert(
    ...
    for selected_element in revit.get_selection():
        ...
        if family:
            if not DB.FamilyUtils.FamilyCanConvertToFaceHostBased(revit.doc, family.Id):
                forms.alert(
                    "Revit API does not support converting this "
            ...
            try:
                with revit.Transaction("Convert to Face Host Based"):
                    ...
                    DB.FamilyUtils.ConvertFamilyToFaceHostBased(revit.doc, family.Id)
