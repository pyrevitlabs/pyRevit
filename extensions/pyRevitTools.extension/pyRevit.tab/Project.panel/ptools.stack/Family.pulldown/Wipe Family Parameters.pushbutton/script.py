...
def reset_param(family_param, reset_message):
    try:
        ...
        # reset value
        if (
            family_param.StorageType == DB.StorageType.Integer
        ...
        with revit.Transaction("Remove all family parameters"):
            for idx, param in enumerate(family_params):
                ...
                if get_elementid_value(param.Id) < 0:
                    message = 'Can not delete builtin "%s"' % param.Definition.Name
                    logger.warning(reset_param(param, message))
                    continue
                # otherwise delete
                try:
                    print('Removing "{}" ({})'.format(param.Definition.Name, param.Id))
