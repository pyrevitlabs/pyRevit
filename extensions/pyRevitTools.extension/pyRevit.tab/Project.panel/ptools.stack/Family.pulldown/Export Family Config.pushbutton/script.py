    ...
    for fsym in (
        ...
        if fsym.Id == symbol_id:
            return FAMILY_SYMBOL_FORMAT.format(
                revit.query.get_name(fsym.Family), revit.query.get_name(fsym)
            )
    ...
    if fparam.StorageType == DB.StorageType.ElementId:
        ...
        if HOST_APP.is_newer_than(2022):  # ParameterType deprecated in 2023
            if DB.SpecTypeId.Boolean.YesNo == fparam.Definition.GetDataType():
                fparam_value = "true" if ftype.AsInteger(fparam) == 1 else "false"
...
def include_type_configs(cfgs_dict, sparams):
    ...
    for ftype in fm.Types:
        ...
        for sparam in sparams:
            ...
            type_config[fparam_name] = get_param_typevalue(ftype, sparam.fparam)
...
def read_configs(selected_fparam_names, include_types=False, include_defaults=False):
    """
...
def store_sharedparam_def(shared_params):
    ...
    for sparam in shared_params:
        ...
        sparamdef_create_options = DB.ExternalDefinitionCreationOptions(
            sparam.Definition.Name, param_type, GUID=sparam.GUID
        )
    ...
    if family_cfg_file:
        ...
        if family_params:
            ...
            inctypes = forms.alert(
                "Do you want to export family types as well?", yes=True, no=True
            )
            ...
            family_configs, shared_parameters = read_configs(
                family_params, include_types=inctypes, include_defaults=incdefault
            )
            ...
            if shared_parameters:
                family_configs[SHAREDPARAM_DEF] = get_shared_param_def_contents(
                    shared_parameters
                )
