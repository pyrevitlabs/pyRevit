...
def reload_links(ref_type=DB.ExternalFileReferenceType.RevitLink):
    ...
    try:
        ...
        links = DB.FilteredElementCollector(revit.doc).OfClass(link_type).ToElements()
        ...
        if links:
            ...
            linkcount = len(xrefs)
            if linkcount > 1:
                selected_xrefs = forms.SelectFromList.show(
                    xrefs,
...
selected_option = forms.CommandSwitchWindow.show(
    linktypes.keys(), message="Select link type:"
)
