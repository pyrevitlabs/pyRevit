...
element_types = (
    DB.FilteredElementCollector(doc).OfClass(DB.ElementType).WhereElementIsElementType()
)

for element_type in element_types:
    # check if element type is arrowhead
    if (
        element_type.get_Parameter(DB.BuiltInParameter.ARROW_CLOSED)
        and element_type.get_Parameter(DB.BuiltInParameter.SYMBOL_NAME_PARAM).AsString()
        != ""
    ):
        # collect arrowhead types and arrowhead types names separately
        arrowhead_types_names.append(
            element_type.get_Parameter(DB.BuiltInParameter.SYMBOL_NAME_PARAM).AsString()
        )
        arrowhead_types.append(element_type)

# The selection form
selection = forms.SelectFromList.show(
    arrowhead_types_names,
    "Select arrowhead types you wand to delete",
    multiselect=True,
    infopanel="Select arrowhead type you wish to delete",
)
