...
def list_attached_constraints(src_element, constraints):
    ...
    for constraint in constraints:
        ...
        if src_element.Id in elids:
            ...
            print(
                "Removing Constraint of Type: {:28} "
                "# of References: {:24} "
            ...
            for t in refs:
                ...
                print(
                    "\t{:35} Linked Object Category: {:20} Id: {}".format(
                        ref.ElementReferenceType.ToString(), elmnt.Category.Name, elid
                    )
                )
    ...
    with revit.Transaction("Remove associated attached_constraints"):
        for attached_constraint in attached_constraints:
            try:
                ...
                logger.error("Failed deleting constraint: %s", attached_constraint)
                continue
else:
