...
class WipeOption:
    ...
    def __repr__(self):
        return "<WipeOption Name:{} Action:{}>".format(self.name, self.wipe_action)
...
for mem in inspect.getmembers(wipeactions):
    moduleobject = mem[1]
    if inspect.isfunction(moduleobject) and hasattr(moduleobject, "is_wipe_action"):
