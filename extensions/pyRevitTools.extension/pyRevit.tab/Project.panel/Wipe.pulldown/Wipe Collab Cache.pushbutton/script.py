...
class CacheModelItem(forms.TemplateListItem):
    ...
    def name(self):
        """Item name"""
        return "{} [ Revit: {} ]{}".format(
            self.item.project, self.item.product, " [ Linked ]" if self.linked else ""
        )
...
for cc in bim360.get_collab_caches():
    # add models
    cmodels.extend([CacheModelItem(x) for x in cc.cache_models])
    # and linked models
    cmodels.extend([CacheModelItem(x, linked=True) for x in cc.cache_linked_models])
