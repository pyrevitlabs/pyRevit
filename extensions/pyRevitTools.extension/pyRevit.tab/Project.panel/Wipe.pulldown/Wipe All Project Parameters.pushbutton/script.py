...
while it.MoveNext():
    ...
    print("-" * 100)
    print(
        "PARAM: {0:<30} UNIT: {1:<10} TYPE: {2:<10} GROUP: {3:<20} BINDING: {4:<10} VISIBLE: {6}\n"
        "APPLIED TO: {5}\n".format(
            name, ut, tp, pg, BIND, [cat.Name for cat in b.Categories], p.Visible
        )
    )
    deflist.append(p)
    ...
    for cat in b.Categories:
        try:
            ...
            if BIND == "Type" and p.Visible:
                ...
                print(
                    "Searching through {0} ElementTypes of Category {1}".format(
                        len(list(elements)), cat.Name
                    )
                )
                for elType in elementTypes:
                    paramidlist.add(
                        get_elementid_value(elType.LookupParameter(p.Name).Id)
                    )
            elif p.Visible:
                print(
                    "Searching through {0} Elements of Category {1}".format(
                        len(list(elements)), cat.Name
                    )
                )
                for el in elements:
