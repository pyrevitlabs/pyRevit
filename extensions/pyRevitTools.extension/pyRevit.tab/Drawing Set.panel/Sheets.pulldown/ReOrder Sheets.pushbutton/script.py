...
item_sample = items[0]
item_params = item_sample.Parameters

name_params = []
for x in item_params:
    name_params.append(x.Definition.Name)
    
order_params = []
for (x,y) in zip(name_params, item_params):
    if "order" in x.lower():
        order_params.append(y)

int_params = []
for x in order_params:
    if x.StorageType == DB.StorageType.Integer:
        int_params.append(x)

   
if not int_params:
    forms.alert("No 'Order' Parameters of the 'Integer' Type were found", title= "Program Cancelled")
