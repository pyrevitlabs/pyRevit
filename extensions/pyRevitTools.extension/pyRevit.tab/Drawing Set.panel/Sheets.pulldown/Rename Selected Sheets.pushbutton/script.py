...
def _collect_rename_inputs(mode):
    if mode == "Find & Replace":
        find_txt = forms.ask_for_string(
            prompt="Find text in current sheet names:", title="Rename Selected Sheets"
        )
        if find_txt is None:
            return None
        if find_txt == "":
        ...
        replace_txt = forms.ask_for_string(
            default="", prompt="Replace with:", title="Rename Selected Sheets"
        )
...
def _print_preview(rename_pairs):
    ...
    for idx, pair in enumerate(rename_pairs):
        ...
        print(
            "[{}] {} -> {}".format(
                pair["sheet"].SheetNumber, pair["old_name"], pair["new_name"]
            )
        )
...
def _report_results(renamed, unchanged, conflicts, failed):
    ...
    if renamed:
        ...
        for item in renamed:
            print(
                "  [{}] {} -> {}".format(
                    item["sheet_number"], item["old_name"], item["new_name"]
                )
            )
    if conflicts:
                ...
                try:
                    ...
                    try:
                        ...
                        restore_reason = (
                            ...
                            "Sheet was kept on temporary name: {}.".format(
                                restore_err, temp_name
                            )
                        )
                    except Exception as temp_restore_err:
                        restore_reason = (
                            " Could not restore original name: {}. "
                            "Could not restore temporary name {} either: {}.".format(
                                restore_err, temp_name, temp_restore_err
                            )
                        )
                failed.append(
                    {
...
def main():
    ...
    selected_option, switches = forms.CommandSwitchWindow.show(
        RENAME_MODES, switches=["Show Report"], message="Select rename option:"
    )
    ...
    while True:
        ...
        existing_names = set(
            name
            for sid, name in all_sheet_names.items()
    ...
    proceed = forms.alert(
        ...
        "See output window for preview.".format(
            len(selected_sheets), len(unique_pairs), len(conflicts)
        ),
        title="Rename Selected Sheets",
        yes=True,
        no=True,
    ...
    forms.alert(
        "Done.\nRenamed: {}\nSkipped (unchanged/conflicts): {}\nFailed: {}".format(
            len(renamed), len(unchanged) + len(conflicts), len(failed)
        ),
        title="Rename Selected Sheets",
    )
