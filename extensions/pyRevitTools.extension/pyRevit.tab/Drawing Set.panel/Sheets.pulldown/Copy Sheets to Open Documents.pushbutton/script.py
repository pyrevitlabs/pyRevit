...
def get_source_sheets():
    sheet_elements = forms.select_sheets(button_name="Copy Sheets", use_selection=True)
...
for dest_doc in dest_docs:
    ...
    for source_sheet in source_sheets:
        print(
            "Copying Sheet: {0} - {1}".format(
                source_sheet.SheetNumber, source_sheet.Name
            )
        )
    ...
    output.print_md(
        "**Copied {} sheets to {} documents.**".format(sheet_count, doc_count)
    )
