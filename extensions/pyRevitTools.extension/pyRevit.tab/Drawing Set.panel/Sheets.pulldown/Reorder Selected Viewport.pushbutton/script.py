...
ViewportConfig = namedtuple("ViewportConfig", ["box_center", "type_id", "view_id"])
...
def capture_vp_config(viewport):
    return ViewportConfig(
        viewport.GetBoxCenter(), viewport.GetTypeId(), viewport.ViewId
    )
    ...
    with revit.Transaction("Reorder Viewports"):
        for vp in vp_list:
            ...
            new_vp = DB.Viewport.Create(
                revit.doc, sheet.Id, config.view_id, config.box_center
            )
...
if selected_vps:
    selected_option = forms.CommandSwitchWindow.show(
        ["Bring to Front", "Send to Back"], message="Select reordering method:"
    )
