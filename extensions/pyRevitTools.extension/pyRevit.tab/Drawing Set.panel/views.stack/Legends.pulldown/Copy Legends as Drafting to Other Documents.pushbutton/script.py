...
if legends:
    for dest_doc in open_docs:
        ...
        for view_type in DB.FilteredElementCollector(dest_doc).OfClass(
            DB.ViewFamilyType
        ):
        ...
        for src_legend in legends:
            ...
            elements_to_copy = []
            for el in DB.FilteredElementCollector(
            ...
            if not elements_to_copy:
                logger.debug(
                    "Skipping empty view: %s", revit.query.get_name(src_legend)
                )
                continue

            # start creating views and copying elements
            with revit.Transaction("Imported Legends as Drafting", doc=dest_doc):
                ...
                for dest, src in zip(copied_elements, elements_to_copy):
                    dest_view.SetElementOverrides(
                        dest, src_legend.GetElementOverrides(src)
                    )
                ...
                if new_name in all_drafting_names:
                    ...
                    logger.warning(
                        'Drafting already exists. Renaming to: "%s"', new_name
                    )
