...
all_legend_names = [
    revit.query.get_name(x) for x in all_graphviews if x.ViewType == DB.ViewType.Legend
]

# get a list of selected legends
drafting_views = forms.select_views(
    title="Select Legends", filterfunc=lambda x: x.ViewType == DB.ViewType.DraftingView
)
...
if drafting_views:
    ...
    if not base_legend:
        forms.alert(
            "At least one Legend view must exist in the model.", exitscript=True
        )
    ...
    for src_drafting in drafting_views:
        ...
        if not elements_to_copy:
            logger.debug("Skipping empty view: %s", revit.query.get_name(src_drafting))
            continue
        ...
        with revit.Transaction("Duplicate Drafting as Legend"):
            ...
            for dest, src in zip(copied_elements, elements_to_copy):
                dest_view.SetElementOverrides(
                    dest, src_drafting.GetElementOverrides(src)
                )
