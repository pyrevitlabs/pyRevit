...
def _adc_diagnose_safe(keynote_ext_path=None):
    ...
    try:
        ...
        _log('INFO', 'Revit %s | .NET %s' % (
            HOST_APP.version,
            'Core (8+)' if is_net8 else 'Framework 4.x'))
    except Exception:
    ...
    _log('INFO', 'api_mode=%s | Public=%s | Legacy=%s'
         % (api_mode, has_public, has_legacy))

    # 3. ADC process check
    try:
        from pyrevit.framework import Process
        adc_procs = [p for p in Process.GetProcesses()
                     if 'DesktopConnector' in p.ProcessName]
        if adc_procs:
            for p in adc_procs:
                try:
                    _log('OK', 'ADC process: %s (PID %d)'
                         % (p.ProcessName, p.Id))
                except Exception:
    ...
    if has_public:
        ...
        try:
            ...
            try:
                ...
                try:
                    _log('INFO', '  ProjectSubscriptionLimit: %s'
        ...
        try:
            ...
            try:
                ...
                active = [s for s in subs
                          if s.State == PubAPI.SubscriptionState.Subscribed]
                _log('OK', 'GetSubscriptions() returned %d total, '
                     '%d active' % (len(subs), len(active)))
                for s in active:
                    try:
                        _log('INFO', '  Drive: %s | Path: %s | '
        ...
        if keynote_ext_path:
            ...
            try:
                ...
                if local:
                    ...
                    _log('OK' if _op.exists(local) else 'FAIL',
    ...
    if has_legacy:
        ...
        _log('INFO', 'Legacy API not loaded — file lock/sync/unlock '
...
def _adc_diagnose_deep(keynote_ext_path=None):
    ...
    for dll in found_dlls:
        ...
        try:
            ...
            _log('OK', 'DLL loaded — types: %s' % ', '.join(
                [t for t in dir(_test_api) if not t.startswith('_')][:15]))
...
def _find_wcf_assemblies():
    ...
    nuget_cache = _op.join(
        _os.environ.get('USERPROFILE', ''), '.nuget', 'packages')
    if _op.isdir(nuget_cache):
        for pkg in ['system.servicemodel.primitives',
    ...
    revit_dir = _op.dirname(HOST_APP.proc_path) if HOST_APP.proc_path \
        else ""
    ...
    targets = {'system.servicemodel.dll',
...
def _try_wcf_shim(api_module, _log):
    ...
    try:
        svc = api_module.DesktopConnectorService()
        _log('OK', 'WCF SHIM WORKED')
        adc.API = api_module
        return svc
    except Exception as ex:
        _log('FAIL', 'Retry FAILED: %s' % ex)
        _log('INFO', 'Confirmed: Legacy API cannot work on .NET 8 '
             'with ADC v2027.')
        try:
            System.AppDomain.CurrentDomain.AssemblyResolve -= _wcf_resolve
        except Exception:
...
class KeynoteManagerWindow(forms.WPFWindow):
    ...
    def _determine_kfile(self):
        ...
        if not self._kfile_ext or self._kfile_handler != 'unknown':
            return

        # --- Step 3: cloud file detected — try ADC resolution ---
        logger.debug('ADC | cloud-hosted keynote, checking ADC...')
        logger.debug('ADC | _api_mode=%s  Public=%s  Legacy=%s'
        ...
        safe_report = _adc_diagnose_safe(
            keynote_ext_path=self._kfile_ext)
        ...
        if _is_deep_diagnostic_enabled():
            ...
            deep_report = _adc_diagnose_deep(
                keynote_ext_path=self._kfile_ext)
