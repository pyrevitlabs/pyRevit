...
if revisions:
    ...
    if set_name is None or set_name.strip() == "":
        forms.alert(
            "Operation cancelled. No name was provided for the revision sheet set."
        )
        script.exit()
    if selected_switch:
        ...
        with revit.Transaction("Create Revision Sheet Set"):
            rev_sheetset = revit.create.create_revision_sheetset(
                revisions, name_format=set_name, match_any=match_any
            )
        ...
        if empty_sheets:
            print(
                "These sheets do not have any model contents and seem to be "
