...
def set_element_overrides(el, ogs, recursive=False):
    ...
    if recursive and isinstance(el, DB.Group):
        for el_nested in el.GetMemberIds():
            set_element_overrides(revit.doc.GetElement(el_nested), ogs, recursive=True)
    revit.doc.ActiveView.SetElementOverrides(el.Id, ogs)
    ...
    with revit.Transaction("Set Element to Dashed Projection Line Pattern"):
        for el in selection:
            if el.ViewSpecific:
                ogs = DB.OverrideGraphicSettings()
                ogs.SetProjectionLinePatternId(
                    DB.LinePatternElement.GetLinePatternElementByName(
                        revit.doc, "Dash"
                    ).Id
                )
                revit.doc.ActiveView.SetElementOverrides(el.Id, ogs)
...
def whiteout():
    # 0xffffff
    colorvg(
        0xFF, 0xFF, 0xFF, projline_only=True, xacn_name="Whiteout Selected Elements"
    )
...
def mark_green():
    # 0x6eeb83
    colorvg(0x6E, 0xEB, 0x83, xacn_name="Mark Selected with Green")


def mark_orange():
    # 0xffb800
    colorvg(0xFF, 0xB8, 0x00, xacn_name="Mark Selected with Orange")


def mark_blue():
    # 0x1be7ff
    colorvg(0x1B, 0xE7, 0xFF, xacn_name="Mark Selected with Yello")
...
selected_switch = forms.CommandSwitchWindow.show(
    options.keys(),
    config={
        "Halftone Selected": {"background": "0xaaaaaa"},
