...
def set_dim_overrides(
    dims,
    txn_name="Dimension Overrides",
    value=None,
    above=None,
    below=None,
    prefix=None,
...
def bake_dim_value():
    with revit.Transaction("Overrride dims value"):
        for seg in grab_dims():
            seg.ValueOverride = "\u200e" + seg.ValueString


def clear_overrides():
    set_dim_overrides(
        grab_dims(),
        txn_name="Reset Dimension Overrides",
        value="",
        above="",
        below="",
        prefix="",
...
def set_to_eq():
    set_dim_overrides(grab_dims(), txn_name="EQ Dims", value="EQ")


def set_to_vfrmfr():
    set_dim_overrides(
        grab_dims(), txn_name="RO VIF Dims", below="VERIFY W/ MFR", suffix="R.O."
    )


def set_to_question():
    set_dim_overrides(grab_dims(), txn_name="(?) Dims", suffix="(?)")
...
def add_plusminus_prefix():
    set_dim_overrides(grab_dims(), txn_name="PlusMinus Dims", prefix="\xb1")


def add_plusminus_suffix():
    set_dim_overrides(grab_dims(), txn_name="PlusMinus Dims", suffix="\xb1")


def set_to_vif_below():
    set_dim_overrides(grab_dims(), txn_name="VIF Dims", below="VIF")


def set_to_vif_suffix():
    set_dim_overrides(grab_dims(), txn_name="VIF Dims", suffix="VIF")


def set_to_typ_below():
    set_dim_overrides(grab_dims(), txn_name="TYP Dims", below="TYP")


def set_to_typ_suffix():
    set_dim_overrides(grab_dims(), txn_name="TYP Dims", suffix="TYP")


def set_to_min_below():
    set_dim_overrides(grab_dims(), txn_name="MIN Dims", below="MIN")


def set_to_min_suffix():
    set_dim_overrides(grab_dims(), txn_name="MIN Dims", suffix="MIN")


def set_to_max_below():
    set_dim_overrides(grab_dims(), txn_name="MAX Dims", below="MAX")


def set_to_max_suffix():
    set_dim_overrides(grab_dims(), txn_name="MAX Dims", suffix="MAX")


def set_to_clr_below():
    set_dim_overrides(grab_dims(), txn_name="CLR Dims", below="CLR")


def set_to_clr_suffix():
    set_dim_overrides(grab_dims(), txn_name="CLR Dims", suffix="CLR")


def set_to_uno_below():
    set_dim_overrides(grab_dims(), txn_name="UNO Dims", below="UNO")


def set_to_uno_suffix():
    set_dim_overrides(grab_dims(), txn_name="UNO Dims", suffix="UNO")


def set_to_pd():
    set_dim_overrides(grab_dims(), txn_name="PD Dims", suffix="P.D.")


def set_to_pd_beow():
    set_dim_overrides(grab_dims(), txn_name="PD Dims", below="P.D.")


def set_to_ro():
    set_dim_overrides(grab_dims(), txn_name="RO Dims", suffix="R.O.")


def set_to_ro_below():
    set_dim_overrides(grab_dims(), txn_name="RO Dims", below="R.O.")
...
selected_switch = forms.CommandSwitchWindow.show(
    options.keys(), message="Pick override option:"
)
