...
exclude_biparams = [
    ...
    DB.BuiltInParameter.ALL_MODEL_IMAGE,
]  # Image
...
def get_param_values(element):
    ...
    for param in element.Parameters:
        ...
        if (
            ...
            and param.Definition.BuiltInParameter not in exclude_biparams
        ):
    ...
    for param in element.Parameters:
        ...
        if (
            ...
            if param_name in value_dict.keys():
                ...
                try:
                    logger.debug(
                        "Setting param: {} to value: {}".format(param_name, param_value)
                    )
                    param.Set(param_value)
                except Exception:
                    logger.debug(
                        "Param: {} Value is not settable: {}".format(
...
with revit.Transaction("Change Element Types"):
    src_element = revit.pick_element("Pick object with source element type")
    if src_element:
