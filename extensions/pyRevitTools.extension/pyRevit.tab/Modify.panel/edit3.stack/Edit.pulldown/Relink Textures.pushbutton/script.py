...
class ConfigurationManager:
    ...
    def show_configuration_dialog(self):
        ...
        while True:
            if current_folders:
                folder_list = "\n".join(
                    "  • {}".format(folder) for folder in current_folders
                )
                ...
                message = (
                    "No search folders configured yet.\n\nWhat would you like to do?"
                )
            ...
            if choice == "Add Folder":
                ...
                if folder and folder not in current_folders:
                    ...
                    forms.alert(
                        "This folder is already in the list.", title="Duplicate Folder"
                    )
                ...
                if current_folders:
                    confirm = forms.alert(
                        "Are you sure you want to remove all {} folder(s)?".format(
                            len(current_folders)
                        ),
...
class TextureIndexer:
    ...
    def build_index(self, roots):
        ...
        output.log_info(
            "Index complete. Found {} texture files.".format(len(self.index))
        )
        return self.index
...
class AssetProcessor:
    ...
    def relink_asset_textures(self, asset, indexer, roots, unresolved):
        ...
        for idx in range(size):
            ...
            for conn_idx in range(conn_count):
                ...
                try:
                    path_prop = connected.FindByName(
                ...
                if isinstance(path_prop, DB.Visual.AssetPropertyString):
                    ...
                    if name:
                        ...
                        if hit and (
                            not current_path or hit.lower() != current_path.lower()
                        ):
                            try:
                                path_prop.Value = hit
                                count += 1
                                ...
                                logger.warning(
                                    "Failed to set path for {}: {}".format(name, error)
                                )
                        else:
                            unresolved.add(name)

                count += self.relink_asset_textures(
                    connected, indexer, roots, unresolved
                )
    ...
    def process_single_asset(self, asset_element, indexer, roots, unresolved):
        ...
        if not asset_element or not isinstance(
            asset_element, DB.AppearanceAssetElement
        ):
        ...
        try:
            ...
            if editable:
                fixed_count = self.relink_asset_textures(
                    editable, indexer, roots, unresolved
                )
                scope.Commit(True)
            else:
                logger.warning(
                    "Failed to start edit scope for asset: {}".format(
                        getattr(asset_element, "Name", "Unknown")
                    )
                )
        except (OSError, IOError, ValueError, AttributeError) as error:
            logger.error(
                "Asset edit failed for {}: {}".format(
...
class TextureRelinker:
    ...
    def run_relink(self):
        ...
        if not assets:
            forms.alert(
                "Failed to collect materials. See output for details.", title="Error"
            )
            return
        ...
        if unresolved:
            logger.warning(
                "Unresolved textures: {} (see list below)".format(len(unresolved))
            )
    ...
    def show_main_menu(self):
        ...
        while True:
            ...
            choice = forms.alert(message, title="Texture Relink Tool", options=options)
...
if __name__ == "__main__":
    try:
        ...
        forms.alert("Script error occurred:\n\n{}".format(str(error)), title="Error")
