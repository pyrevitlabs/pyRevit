...
acceptable_curves = (DB.Arc, DB.Ellipse, DB.NurbSpline)

detail_line_types = [DB.DetailLine, DB.DetailEllipse, DB.DetailArc, DB.DetailNurbSpline]
...
class MakePatternWindow(forms.WPFWindow):
    ...
    def pat_scale_multiplier(self):
        ...
        if pyutils.isnumber(mult):
            ...
            logger.debug("Multiplier is not a number (%s). Defaulting to 1.0", mult)
            return 1.0
    ...
    def pat_rotation(self):
        ...
        if pyutils.isnumber(rotat):
            ...
            logger.debug("Rotation is not a number (%s). Defaulting to 0.0", rotat)
            return 0.0
    ...
    def update_fillgrid(self, rvt_fillgrid, scale):
        ...
        rvt_fillgrid.Origin = DB.UV(ext_origin.U * scale, ext_origin.V * scale)
    ...
    def cleanup_selection(self, rvt_elements, for_model=True):
        ...
        for element in rvt_elements:
            if isinstance(element, acceptable_lines):
                ...
                bg_fillpat = revit.query.get_fillpattern_from_element(element)
                fg_fillpat = revit.query.get_fillpattern_from_element(
    ...
    def setup_patnames(self):
        ...
        self._existing_modelpats = sorted(
            [
                x.Name
                for x in fillpats
        ...
        self._existing_draftingpats = sorted(
            [
                x.Name
                for x in fillpats
    ...
    def setup_view_scale(self):
        ...
        self.viewscale_tb.Text = revit.active_view.Parameter[biparam].AsValueString()

    def pick_domain(self):
        # ask user for origin and max domain points
        with forms.WarningBar(
            title="Pick origin point (bottom-left corner of the pattern area):"
        ):
            view = revit.active_view
            if not view.SketchPlane and not isinstance(view, DB.ViewDrafting):
                base_plane = DB.Plane.CreateByNormalAndOrigin(
        ...
        if pat_bottomleft:
            with forms.WarningBar(title="Pick top-right corner of the pattern area:"):
    ...
    def make_pattern_line(self, start_xyz, end_xyz):
        ...
        if vdir.Z == 1.0:
            return (
                MakePatternWindow.round_coord(start_xyz.X),
    ...
    def create_pattern(self, domain, export_only=False, export_path=None):
        ...
        self.selected_geom_curves, self.selected_fillgrids = self.cleanup_selection(
            self._selection, for_model=self.is_model_pat
        )
        ...
        for geom_curve in self.selected_geom_curves:
            if isinstance(geom_curve, acceptable_curves):
                ...
                line_tuples.append(
                    self.make_pattern_line(
                        geom_curve.GetEndPoint(0), geom_curve.GetEndPoint(1)
                    )
                )

        call_params = (
            "Name:{} Model:{} FilledRegion:{} Domain:{}\nLines:{}\nFillGrids:{}".format(
                self.pat_name,
                self.is_model_pat,
    ...
    def verify_name(self):
        if not self.pat_name.strip():
            ...
            forms.alert(
                'Read-Only pattern with name "{}" already exists '.format(self.pat_name)
            )
            return False
        return True
    ...
    def export_pat(self, sender, args):
        if self._export_only:
            ...
            if domain and export_dir:
                self.create_pattern(domain, export_only=True, export_path=export_dir)
            self.Close()
...
if __name__ == "__main__":
    MakePatternWindow("MakePatternWindow.xaml", selection.elements).show(modal=True)
