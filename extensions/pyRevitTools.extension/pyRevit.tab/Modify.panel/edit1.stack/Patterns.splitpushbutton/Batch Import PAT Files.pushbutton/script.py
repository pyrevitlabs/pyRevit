...
PatDef = namedtuple("PatDef", ["name", "comments", "version", "units", "type", "grids"])
...
def make_fillgrid(gridstring, scale=0.00328084):
    ...
    if griddata:
        ...
        rvt_fill_grid.Origin = DB.UV(griddata[1] * scale, griddata[2] * scale)
...
def make_pattern(patdef):
    ...
    if existing_fpelement:
        ...
        logger.debug(
            "Updated FillPatternElement with id:{}".format(existing_fpelement.Id)
        )
    else:
    ...
    with revit.Transaction("Batch import PAT files"):
        for patfile in patfiles:
            ...
            for patdef in patdefs:
                print(
                    'Processing "{}" @ "{}" ({}, {})'.format(
                        patdef.name, patfile, patdef.units, patdef.type
                    )
                )
                make_pattern(patdef)
    print("Done...")
