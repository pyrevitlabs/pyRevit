...
def line_count(document=doc):
    ...
    try:
        ...
        for line_count, view_id_int in sorted(
            zip(detail_lines.values(), detail_lines.keys()), reverse=True
        ):
            view_id = DB.ElementId(view_id_int)
            view_creator = DB.WorksharingUtils.GetWorksharingTooltipInfo(
                document, view_id
            ).Creator
            try:
            ...
            table_data.append(
                [line_count, view_name, output.linkify(view_id), view_creator]
            )
        table_data.append(
            [
                str(sum(detail_lines.values())) + " Lines in Total",
                "In " + str(len(detail_lines)) + " Views",
                "",
                "",
            ]
        )
        output.print_table(
            table_data,
            columns=["Number", "Name", "Id", "Creator"],
            last_line_style="font-weight:bold;font-size:1.2em;",
        )
...
output.print_md(
    "By: [{}]({}) and some improvements from [{}]({})\n\n".format(
        __author__, __contact__, __author2__, __contact2__
    )
)
