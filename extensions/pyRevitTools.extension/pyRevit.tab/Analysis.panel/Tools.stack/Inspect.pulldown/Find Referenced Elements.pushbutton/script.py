...
if not selection.is_empty:
    print(
        "Searching for all objects tied to ELEMENT ID: {0}...".format(
            selection.first.Id
        )
    )
    ...
    for elId in linked_elements_list:
        ...
        if el and elId in selection.element_ids:
            ...
            print(
                "ID: {}\t\tTYPE: {} ( selected object )".format(
                    elid_link, el.GetType().Name
                )
            )
        elif el:
            elid_link = output.linkify(elId)
            if isinstance(el, DB.FamilyInstance):
                ...
                print("ID: {}\t\tTYPE: {}".format(elid_link, el.GetType().Name))
