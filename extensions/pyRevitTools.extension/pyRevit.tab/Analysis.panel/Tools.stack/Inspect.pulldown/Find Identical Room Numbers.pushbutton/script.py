...
if revit.active_view:
    ...
    duplicates = [item for item, count in Counter(room_numbers).items() if count > 1]
        ...
        for room_number in duplicates:
            ...
            for room_id in rooms:
                ...
                if rm.Number == room_number:
                    room_name = rm.Parameter[DB.BuiltInParameter.ROOM_NAME].AsString()
                    print(
                        "\t{} (@ {}) {}".format(
                            room_name.ljust(30), rm.Level.Name, output.linkify(rm.Id)
                        )
                    )
            print("\n")
    else:
