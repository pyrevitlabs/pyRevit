...
views = filter(
    lambda x: (
        (x.ViewType != DB.ViewType.DraftingView or x.ViewType != DB.ViewType.ThreeD)
...
for v in views:
    print(
        "Searching {} of type: {}".format(
            revit.query.get_name(v), str(v.ViewType).ljust(25)
        )
    )

    # collect view elements
    cl_els = (
        DB.FilteredElementCollector(revit.doc, v.Id)
        .WhereElementIsNotElementType()
        .ToElementIds()
    )
