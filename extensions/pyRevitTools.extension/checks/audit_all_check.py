...
class ElementCounts:
    ...
    def __init__(self, document):
        ...
        self.purgeable_elements_count = count_purgeable_elements(document) if document else 0
        self.activated_analytical_model_elements_count = count_analytical_model_activated(document) if document else 0

class WarningsInfo:
    """Handles warning metrics."""
    def __init__(self, document):
        self.all_warnings_count = get_warnings_count(document) if document else 0
        self.critical_warnings_count = get_critical_warnings_count(document, CRITICAL_WARNINGS) if document else 0
...
class SheetViewInfo:
    ...
    def __init__(self, document, sheets_set, views):
        ...
        self.views_not_on_sheets = count_unplaced_views(sheets_set, self.views_count) if sheets_set and views else 0
        self.schedule_count = count_total_schedules(document) if document else 0
        self.schedules_not_sheeted_count = count_unplaced_schedules(document) if document else 0
...
class ViewTemplateFilterInfo:
    ...
    def __init__(self, document, views):
        self.view_templates_count = len(get_all_view_templates(document)) if document else 0
        self.unused_view_templates_count = count_unused_view_templates(views) if views else 0
        self.all_filters_count = count_filters(document) if document else 0
        self.unused_view_filters_count = count_unused_filters_in_views(views) if views else 0

class MaterialsLinePatternInfo:
    """Handles materials and line patterns."""
    def __init__(self, document):
        self.materials_count = len(get_elements_by_categories([DB.BuiltInCategory.OST_Materials], doc=document)) if document else 0
        self.line_patterns_count = len(get_elements_by_class(DB.LinePatternElement, doc=document)) if document else 0
...
class LinksInfo:
    ...
    def __init__(self, document, rvtlinks_elements_items):
        ...
        self.rvtlinks_unpinned_count = count_unpinned_revit_links(rvtlinks_elements_items) if rvtlinks_elements_items else 0

class FamilyInfo:
    """Handles family-related metrics."""
    def __init__(self, document):
        self.inplace_family_count = count_in_place_families(document) if document else 0
        self.not_parametric_families_count = count_non_parametric_families(document) if document else 0
        self.family_count = count_total_families(document) if document else 0
        self.imports_subcats_count = count_import_subcategories(document) if document else 0
        self.generic_models_types_count = len(get_families(document)) if document else 0
        self.detail_components_count = count_detail_components(document) if document else 0
        self.detail_lines_count = count_detail_lines(document) if document else 0

class TextNotesInfo:
    """Manages text note metrics."""
    def __init__(self, document):
        self.text_notes_types_count = count_total_textnote_types(document) if document else 0
        self.text_notes_types_wf_count = count_textnote_types_with_changed_width_factor(document) if document else 0
        self.text_bg_count = count_textnote_types_with_opaque_background(document) if document else 0
        self.text_notes_count = count_text_notes(document) if document else 0
        self.text_notes_caps_count = count_text_notes_with_all_caps(document) if document else 0

class GroupInfo:
    """Handles group-related data."""
    def __init__(self, document):
        self.detail_groups_count = count_detail_groups_types(document) if document else 0
        self.detail_groups_types_count = count_detail_group_instances(document) if document else 0
        self.model_group_count = count_model_groups_types(document) if document else 0
        self.model_group_type_count = count_model_group_instances(document) if document else 0

class ReferencePlaneInfo:
    """Manages reference plane metrics."""
    def __init__(self, document):
        self.reference_planes_count = count_reference_planes(document) if document else 0
        self.unnamed_ref_planes_count = count_unnamed_reference_planes(document) if document else 0
...
class DimensionsInfo:
    ...
    def __init__(self, document):
        ...
        self.dim_overrides_count = count_dimension_overrides(document) if document else 0
...
class ReportData:
    _KEY_MAPPING = OrderedDict([
        ...
        ("activated_analytical_model_elements_count", "element_counts.activated_analytical_model_elements_count"),
        ...
        ("schedules_not_sheeted_count", "sheet_view_info.schedules_not_sheeted_count"),
        ("copied_views_count", "sheet_view_info.copied_views_count"),
        ("view_templates_count", "view_template_filter_info.view_templates_count"),
        ("unused_view_templates_count", "view_template_filter_info.unused_view_templates_count"),
        ("all_filters_count", "view_template_filter_info.all_filters_count"),
        ("unused_view_filters_count", "view_template_filter_info.unused_view_filters_count"),
        ...
        ("not_parametric_families_count", "family_info.not_parametric_families_count"),
        ...
        ("unnamed_ref_planes_count", "reference_plane_info.unnamed_ref_planes_count"),
    ...
    def __init__(self, document=None):
        ...
        self.rvtlinks_elements_items = get_linked_model_instances(document).ToElements() if document else None
    ...
    def export_to_csv(self, export_file_path=EXPORT_FILE_PATH, headers=None):
        ...
        if not isfile(export_file_path):
            ...
            with open(export_file_path, mode="rb") as csv_file:
                ...
                flag = any(
                    row[1] == self.metadata.date and row[2] == self.document_info.clean_name for row in r
                )
            if not flag:
...
def generate_html_content(data, links_cards=""):
    ...
    critical_elements_frame = create_frame(
        ...
        card_builder(1000, data.element_counts.purgeable_elements_count, " Purgeable (2024+)"),
        card_builder(100, data.warnings_info.all_warnings_count, " Warnings"),
        card_builder(5, data.warnings_info.critical_warnings_count, " Critical Warnings"),
        card_builder(0, data.element_counts.activated_analytical_model_elements_count, " Analytical Model ON"),
        links_cards,
    )
    ...
    sheets_views_graphics_frame = create_frame(
        ...
        card_builder(300, data.sheet_view_info.views_not_on_sheets, " Views not on Sheets"),
        card_builder(20, data.sheet_view_info.schedule_count, " Schedules"),
        card_builder(5, data.sheet_view_info.schedules_not_sheeted_count, " Schedules not on sheet"),
        card_builder(0, data.sheet_view_info.copied_views_count, " Copied Views"),
        card_builder(100, data.view_template_filter_info.view_templates_count, " View Templates"),
        card_builder(0, data.view_template_filter_info.unused_view_templates_count, " Unused VT"),
        card_builder(0, data.view_template_filter_info.all_filters_count, " Filters"),
        card_builder(0, data.view_template_filter_info.unused_view_filters_count, " Unused Filters"),
    )
    ...
    families_frame = create_frame(
        ...
        card_builder(100, data.family_info.not_parametric_families_count, " Non-Parametric Families"),
        card_builder(0, data.family_info.imports_subcats_count, " Imports in Families"),
        card_builder(50, data.family_info.generic_models_types_count, " Generic Models Types"),
        card_builder(100, data.family_info.detail_components_count, " Detail Components"),
    )
    graphical2d_elements_frame = create_frame(
        ...
        card_builder(30, data.materials_line_pattern_info.line_patterns_count, " Line Patterns"),
        card_builder(30, data.text_notes_info.text_notes_types_count, " Text Notes Types"),
        card_builder(1, data.text_notes_info.text_bg_count, " Text Notes w/ White Background"),
        card_builder(0, data.text_notes_info.text_notes_types_wf_count, " Text Notes Width Factor !=1"),
        card_builder(2000, data.text_notes_info.text_notes_count, " Text Notes"),
        card_builder(100, data.text_notes_info.text_notes_caps_count, " Text Notes allCaps"),
        ...
        card_builder(0, data.dimensions_info.dim_overrides_count, " Dimension Overrides"),
        card_builder(100, data.revisions_info.revision_clouds_count, " Revision Clouds"),
    )
    groups_summary_frame = create_frame(
        ...
        card_builder(20, data.group_info.detail_groups_types_count, " Detail Group Types"),
    )
    reference_planes_frame = create_frame(
        "Reference Planes",
        card_builder(100, data.reference_plane_info.reference_planes_count, " Ref Planes"),
        card_builder(10, data.reference_plane_info.unnamed_ref_planes_count, " Ref Planes no_name"),
    )
    materials_frame = create_frame(
        "Materials", card_builder(100, data.materials_line_pattern_info.materials_count, " Materials")
    )
...
def audit_document(doc, output):
    try:
        ...
        if data.links_info.rvtlinks_count > 0:
            ...
            links_cards = card_builder(50, data.links_info.rvtlinks_count, " Links") + card_builder(
                0, data.links_info.rvtlinks_unpinned_count, " Links not pinned"
            )

        output.print_md("# <p style='text-align: center;'>" + data.document_info.clean_name + "</p>")
...
def generate_rvt_links_report(link_document_data, output):
    ...
    if link_document_data.rvtlinks_elements_items:
        links_data = card_builder(50, link_document_data.links_info.rvtlinks_count, " Links") + card_builder(
            0, link_document_data.links_info.rvtlinks_unpinned_count, " Links not pinned"
        )
