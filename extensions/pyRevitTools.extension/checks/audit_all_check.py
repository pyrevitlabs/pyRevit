...
def check_model(doc, output):
    try:
        ...
        if rvtlinks_elements_items:
            ...
            for idx, link_doc in enumerate(rvtlinks_docs):
                project_name, project_number, project_client = project_informations(link_doc)
                link_data.append([project_name, project_number, project_client, phases(link_doc), worksets(link_doc), links_names[idx], links_instances_names[idx], type_status[idx], pinned[idx]])
            columns_headers = [
                'Project Name', 'Project Number', 'Client Name', 'Project Phases',
                'Worksets', 'Linked File Name', 'Instance Name', 'Loaded Status', 'Pinned status'
            ]
        ...
        # RVTLinks
        if rvtlinks_elements_items:
            generate_rvt_links_report(output, rvtlinks_docs, body_css)
