...
def make_title(base_point, fr_type):
    tnote_typeid = revit.doc.GetDefaultElementTypeId(DB.ElementTypeGroup.TextNoteType)
    DB.TextNote.Create(
        revit.doc,
...
def make_filledregion_element(base_point, fr_type):
    ...
    DB.FilledRegion.Create(
        revit.doc, fr_type.Id, revit.active_view.Id, List[DB.CurveLoop]([cloop])
    )
...
with revit.Transaction("Generate FilledRegion Swatched"):
    for idx, filledregion_type in enumerate(
        sorted(filledregion_types, key=lambda x: revit.query.get_name(x))
    ):
