from pyrevit.coreutils.git import git, get_repo, RepoInfo
from pyrevit.coreutils.logger import get_logger, stdout_hndlr
from pyrevit.extensions.extmanager import get_installed_ui_extensions, get_installed_lib_extensions

from pyrevit.repo import get_pyrevit_repo
...
def get_thirdparty_lib_repos():
    ...
    for root_dir in user_config.get_ext_root_dirs():
        lib_pkg_info_list = get_installed_lib_extensions(root_dir)
    ...
    for lib_pkg_info in lib_pkgs:
        repo_info = get_repo(lib_pkg_info.directory)
        if repo_info:
...
def get_thirdparty_pkg_repos():
    ...
    for root_dir in user_config.get_ext_root_dirs():
        pkg_info_list = get_installed_ui_extensions(root_dir)
    ...
    for pkg_info in pkgs:
        repo_info = get_repo(pkg_info.directory)
        if repo_info:
