...
Let's talk basics:
    1. Revit Addons are written in C# and are windows ``.dll`` files.
    2. pyRevit is written as an IronPython module. (a bit more complex than that)
    3. Revit doesn't have an option to run external python scripts.
...
So:
    * ``pyRevitLoader.dll`` is a simple C# addin for Revit
    * ``pyRevitLoader.dll`` loads ``pyRevitLoader.py`` at startup.
    * ``pyRevitLoader.py`` sets up the environment and loads pyRevit.


It's that simple really. See the sources below. From here on, the documentation page for the `pyrevit.loader` module will take you through all the steps of parsing extensions, making dll assemblies and creating the user interface for the parsed extensions.
