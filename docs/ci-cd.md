...
- **WinGet validation fails with `0x8A150056` / `elevationProhibited`**: WinGet's validation VM runs as an administrator; manifests that declare `ElevationRequirement: elevationProhibited` on user-scope installers fail before Inno runs (`APPINSTALLER_CLI_ERROR_INSTALLER_PROHIBITS_ELEVATION`). The `winget` pipeline strips that field after `wingetcreate update` and before `wingetcreate submit`. Inno installers still use `PrivilegesRequired=lowest` for real per-user installs. Re-run validation with `@wingetbot run` on the winget-pkgs PR after the manifest is fixed. For same-version resubmits, patch the open winget-pkgs PR or dispatch **`Update Winget manifests`** without `winget_replace_version`. Use `winget_replace_version` only when replacing an older catalog version with a newer one (values must differ).
